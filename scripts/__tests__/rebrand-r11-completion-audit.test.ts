...
describe('R.11 completion audit', () => {
  ...
  test('keeps the historical T441 audit snapshot while current mapping records R.11 closeout', () => {
    const audit = readFileSync(auditPath, 'utf8')
    const currentMainValidation =
      audit.split('## Current Main Validation Matrix')[1]?.split('## Current Blockers')[0] ?? ''
  ...
  test('records the current consolidation backlog as the execution surface', () => {
    ...
    expect(backlog).toContain('PR queue status: cleared after PR #222 merged')
    expect(backlog).toContain('#222 `fix/post-218-validation-closeout`')
    expect(backlog).toContain('158')
    expect(backlog).toContain('157 non-main/non-R.11-backup')
    expect(backlog).toContain('0 open PR branches')
    expect(backlog).toContain('157 origin branch review candidates')
  ...
  test('records a unified remaining-worklist for continuing R.11', () => {
    ...
    expect(backlog).toContain('Remote branches requiring review: 157')
  ...
  test('records volatile preflight context without destructive authorization', () => {
    ...
    expect(currentBlockers).toContain('is green')
    expect(currentBlockers).toContain('PR #222 merged')
    ...
    expect(currentBlockers).toContain('git status --porcelain is not empty')
    expect(currentBlockers).toContain('.omc/state/last-tool-error.json')
    expect(currentBlockers).toContain('.omx/context/wave-v13-final-5-20260514T120000Z.md')
    ...
    expect(preflightContextInventory).toContain('PR #222')
    expect(preflightContextInventory).toContain('fd22607d')
    expect(preflightContextInventory).toContain('Current checkout: `docs/r11-post-222-blocker-refresh`')
    ...
    expect(preflightContextInventory).toContain('Worktree clean: no')
  ...
  test('records the current fork-review blocker count', () => {
    ...
    expect(hardPrereqs).toContain('GitHub reports 2 fork(s); expected 0')
    expect(hardPrereqs).toContain('Blocked')
    expect(currentBlockers).toContain('fork-review')
    expect(currentBlockers).toContain('GitHub reports 2 fork(s); expected 0')
    ...
    expect(forkReviewInventory).toContain('Current fork count: 2')
    expect(forkReviewInventory).toContain('Expected fork count: 0')
    expect(forkReviewInventory).toContain('dofaromg/rox-one-terminal')
    expect(forkReviewInventory).toContain('agisotadev/rox-one-terminal')
  ...
  test('records an operator-ready fork review decision manifest', () => {
    ...
    expect(manifest).toContain('Current fork count: 2')
    expect(manifest).toContain('Expected fork count: 0')
    expect(manifest).toContain('`dofaromg/rox-one-terminal`')
    expect(manifest).toContain('`agisotadev/rox-one-terminal`')
    ...
    expect(manifest).toContain('ROX_R11_EXPECTED_FORKS=2')
  ...
  test('records the current remote branch review blocker count', () => {
    ...
    expect(currentBlockers).toContain('157 non-main/non-R.11-backup origin branches')
    expect(currentBlockers).toContain('docs/release/r11-remote-branch-review-2026-05-14.md')
    expect(remoteBranchReview).toContain('Total origin heads: 158')
    expect(remoteBranchReview).toContain('Non-main/non-R.11-backup origin branches: 157')
  ...
  test('records an operator-ready remote branch retirement manifest', () => {
    ...
    expect(manifest).toContain('Non-main/non-R.11-backup origin branches: 157')
    expect(manifest).toContain('Open PR branches: 0')
    expect(manifest).toContain('Merged PR branch heads: 139')
    expect(manifest).toContain('Closed/unmerged PR branch heads: 9')
    expect(manifest).toContain('No-visible-PR branch heads: 8')
  ...
  test('separates operator-owned unblocks from destructive authorization', () => {
    ...
    for (const required of [
      ...
      '157 non-main/non-R.11-backup origin branches',
      '0 open PR branches',
