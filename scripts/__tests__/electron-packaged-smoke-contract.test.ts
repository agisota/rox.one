...
describe('packaged Electron smoke contract', () => {
  it('uses clean process exit as packaged readiness proof without stale required markers', () => {
    ...
    expect(packagedSmoke).not.toContain("'ROX_SERVER_URL=': false");
    expect(packagedSmoke).toContain("replace(/ROX_SERVER_TOKEN=\\S+/g, 'ROX_SERVER_TOKEN=[REDACTED]')");
    expect(packagedSmoke).toContain("replace(/ROX_SERVER_URL=\\S+/g, 'ROX_SERVER_URL=[REDACTED]')");
