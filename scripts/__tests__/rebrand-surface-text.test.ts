...
describe('R.1 surface text rebrand', () => {
  it('renames legacy i18n keys and consumers', () => {
    ...
    for (const relativePath of localeFiles) {
      ...
      expect(content).not.toContain('"menu.roxMenu"')
      expect(content).not.toContain('"onboarding.apiSetup.roxAgentsBackend"')
  ...
  it('keeps active README body copy free of legacy product prose', () => {
    ...
    expect(activeBody).not.toContain('Rox Agents')
    expect(activeBody).not.toContain('Rox Agent')
  })
  ...
  it('uses the ROX log path in active automations docs', () => {
    ...
    expect(automations).not.toContain('~/.rox-agent/logs/messaging-gateway.log')
  ...
  it('uses ROX.ONE in messaging playground demo labels', () => {
    ...
    for (const relativePath of demoFiles) {
      const content = readRepoFile(relativePath)
      expect(content).not.toContain('Rox Agents')
      expect(content).not.toContain('Rox Agent')
    }
