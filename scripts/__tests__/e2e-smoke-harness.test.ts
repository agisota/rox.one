...
describe('RC E2E smoke harness', () => {
  ...
  it('recognizes S09 upstream ROX flows and documents current validation paths', async () => {
    const harness = await import('../e2e-smoke')
    const scenario = harness.resolveRequiredScenario('s09-upstream-rox-flows')
    const ticket = readFileSync(join(rootDir, 'docs/tickets/T347-rc-s09-upstream-base-rox-custom-flows.md'), 'utf8')

    expect(scenario.command.slice(0, 2)).toEqual(['bun', 'test'])
    expect(scenario.command).toContain('scripts/__tests__/protected-rox-paths.test.ts')
    expect(scenario.command).toContain('packages/shared/src/config/__tests__/storage-scope-auth.test.ts')
    expect(scenario.command).toContain('packages/shared/src/config/__tests__/storage-scope-runtime.test.ts')
    expect(scenario.command).toContain('packages/shared/src/config/__tests__/storage-multi-tenant-migration.test.ts')
    expect(scenario.command).toContain('packages/shared/src/config/__tests__/storage-scope.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/storage/__tests__/object-storage.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/server-core-rpc-storage-scope.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/workspace-scope.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/workspace-rbac-wire.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/rbac-e2e.test.ts')
    expect(scenario.command).toContain('packages/shared/src/auth/__tests__/policy-engine.test.ts')
    expect(scenario.command).toContain('packages/shared/src/auth/__tests__/scope-forgery.property.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/roles.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/roles-audit.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/webui/__tests__/account-session-boundary.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/webui/__tests__/account-http.test.ts')
    expect(scenario.command).toContain(
      'apps/electron/src/renderer/components/app-shell/input/__tests__/prompt-rewrite-flow.test.ts',
    )
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/review-board.test.ts')
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/validation-gates.test.ts')
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/experience-layer.test.ts')
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/experience-layer-registry.test.ts')
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/experience-layer-security.test.ts')
    expect(scenario.command).toContain('packages/shared/src/workbench/__tests__/experience-layer-e2e-scenario.test.ts')
    expect(scenario.command).toContain('packages/server-core/src/handlers/rpc/__tests__/experience-rpc.test.ts')
    expect(scenario.command).toContain('apps/electron/src/renderer/experience-layer/__tests__/ipc-bridge.test.ts')

    expect(ticket).toContain('bun run e2e:smoke -- --scenario s09-upstream-rox-flows')
    expect(ticket).not.toContain('packages/shared/src/config/__tests__/**')
    expect(ticket).not.toContain('packages/shared/src/auth/__tests__/**')
    expect(ticket).not.toContain('packages/shared/src/**/__tests__/experience*.test.ts')
  })
})
