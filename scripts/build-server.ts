 ...
 * not node: builtins, not workspace @rox-agent/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@rox-agent/')) continue;
...
function copyWorkspacePackages(config: ServerBuildConfig): void {
  ...
  // ROX_MESSAGING_WA_WORKER env resolution in packages/server/src/index.ts.
  const packages = [
    'server',
    'server-core',
    'shared',
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  // Root package.json with workspaces (Bun resolves @rox-agent/* through this)
  const rootPkg = {
    name: 'rox-server-dist',
    version,
    private: true,
    workspaces: ['packages/*'],
  };
  ...
  // Create workspace symlinks in node_modules/@rox-agent/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@rox-agent');
  ...
  if (existsSync(packagesDir)) {
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@rox-agent/')) {
          const shortName = name.replace('@rox-agent/', '');
...
function createEntryScripts(config: ServerBuildConfig): void {
  ...
  // bin/rox-server — main entry wrapper
  const roxServer = `#!/bin/sh
set -e
...
export ROX_BUNDLED_ASSETS_ROOT="$ROOT"
export ROX_IS_PACKAGED=true
export ROX_APP_ROOT="$ROOT"
export ROX_RESOURCES_PATH="$ROOT/resources"

# CLI tools (doc tools use uv + Python scripts)
export ROX_UV="$ROOT/resources/bin/uv"
export ROX_SCRIPTS="$ROOT/resources/scripts"
...
`;
  writeFileSync(join(binDir, 'rox-server'), roxServer);
...
chmod +x "$DIR/bin/rox-server" "$DIR/start.sh"
...
# ROX_RPC_TLS_CERT=/path/to/cert.pem
# ROX_RPC_TLS_KEY=/path/to/key.pem
# ROX_RPC_TLS_CA=/path/to/ca.pem
ENVFILE
  echo ""
...
if [ "\${1:-}" = "--systemd" ]; then
  ...
  SERVICE_USER="\${ROX_USER:-\$(logname 2>/dev/null || echo rox)}"
  SERVICE_FILE="/etc/systemd/system/rox-server.service"
...
Environment=ROX_RPC_HOST=127.0.0.1
Environment=ROX_RPC_PORT=9100
ExecStart=$DIR/bin/rox-server
Restart=on-failure
RestartSec=5
...
UNIT
  ...
  systemctl enable rox-server

  echo ""
  echo "Systemd service installed."
  echo "  Start:   sudo systemctl start rox-server"
  echo "  Status:  sudo systemctl status rox-server"
  echo "  Logs:    journalctl -u rox-server -f"
  echo ""
  exit 0
fi
...
# Make binaries executable
RUN chmod +x bin/rox-server vendor/bun/bun resources/bin/uv && \\
    for f in resources/bin/*; do [ -f "$f" ] && chmod +x "$f"; done

ENV ROX_IS_PACKAGED=true
ENV ROX_BUNDLED_ASSETS_ROOT=/app
ENV ROX_APP_ROOT=/app
ENV ROX_RESOURCES_PATH=/app/resources
ENV ROX_UV=/app/resources/bin/uv
ENV ROX_SCRIPTS=/app/resources/scripts
ENV ROX_RPC_HOST=0.0.0.0
ENV ROX_RPC_PORT=9100
ENV PATH="/app/resources/bin:/app/vendor/bun:\${PATH}"

EXPOSE 9100

ENTRYPOINT ["/app/bin/rox-server"]
`;
...
async function main(): Promise<void> {
  ...
  if (config.compress) {
    const archiveName = `rox-server-${version}-${platform}-${arch}.tar.gz`;
