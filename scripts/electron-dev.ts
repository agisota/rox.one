...
function detectInstance(): void {
  ...
  if (match) {
    ...
    process.env.CRAFT_APP_NAME = `ROX.ONE [${instanceNum}]`;
    process.env.CRAFT_CONFIG_DIR = join(process.env.HOME || "", `.rox-${instanceNum}`);
...
function getElectronEnv(): Record<string, string> {
  ...
  return {
    ...
    CRAFT_APP_NAME: process.env.CRAFT_APP_NAME || "ROX.ONE",
