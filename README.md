...
ROX_SERVER_TOKEN=$(openssl rand -hex 32) bun run packages/server/src/index.ts
```

The server prints the connection details on startup:

```
ROX_SERVER_URL=ws://203.0.113.5:9100
ROX_SERVER_TOKEN=<generated-token>
```
...
```bash
ROX_SERVER_URL=wss://203.0.113.5:9100 ROX_SERVER_TOKEN=<token> bun run electron:start
```
...
|----------|----------|---------|-------------|
| `ROX_SERVER_TOKEN` | Yes | — | Bearer token for client authentication |
| `ROX_RPC_HOST` | No | `127.0.0.1` | Bind address (`0.0.0.0` for remote access) |
| `ROX_RPC_PORT` | No | `9100` | Bind port |
| `ROX_RPC_TLS_CERT` | No | — | Path to PEM certificate file (enables `wss://`) |
| `ROX_RPC_TLS_KEY` | No | — | Path to PEM private key file (required with cert) |
...
ROX_RPC_HOST=0.0.0.0 \
ROX_RPC_TLS_CERT=certs/cert.pem \
ROX_RPC_TLS_KEY=certs/key.pem \
bun run packages/server/src/index.ts
```

The server will print `ROX_SERVER_URL=wss://<your-public-ip>:9100`.
...
export ROX_SERVER_URL=ws://127.0.0.1:9100
export ROX_SERVER_TOKEN=<your-token>
...
**Prompt actions** create a new agent session with a prompt. They support `@mentions` for sources and skills, and environment variables like `$ROX_LABEL` and `$ROX_SESSION_ID` are expanded automatically.
...
ROX.ONE is a white-label fork of the upstream Rox Agents OSS project at
https://github.com/lukilabs/rox-agents-oss. Apache 2.0 attribution is
