...
Rox Agents uses two different agent backends:
...
- **Codex** — powered by the [Codex app-server](https://github.com/lukilabs/rox-agents-codex), which communicates via JSON-RPC over stdio. Codex connections are limited to **direct OpenAI API** (via API key or ChatGPT subscription OAuth).
...
Or configure manually in `~/.rox-agent/workspaces/{id}/hooks.json`:
    ...
    "LabelAdd": [
      {
        ...
        "hooks": [
          { "type": "command", "command": "osascript -e 'display notification \"Urgent session\" with title \"Rox Agent\"'" }
        ]
      }
    ]
  }
...
**Two hook types:**
- **Command hooks** — run shell commands with event data as environment variables (`$ROX_LABEL`, `$ROX_SESSION_ID`, etc.)
...
See the [Hooks documentation](https://agents.rox.do/docs/hooks/overview) for the full reference.
