...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rox:restore-input', handler)
    return () => window.removeEventListener('rox:restore-input', handler)
  }, [sessionId])
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `roxagents://${route}${separator}window=focused`
    try {
      ...
      <StyledDropdownMenuContent align="end" sideOffset={8}>
        {sharedUrl ? (
          <>
            ...
            <StyledDropdownMenuSeparator />
            <StyledDropdownMenuItem onClick={() => window.electronAPI.openUrl('https://agents.rox.do/docs/go-further/sharing')}>
