...
import { Icon_Home, Icon_Folder, Spinner } from '@rox-agent/ui'

import * as storage from '@/lib/local-storage'
import { extractWorkspaceSlugFromPath } from '@rox-agent/shared/utils/workspace-slug'
...
import { Tooltip, TooltipContent, TooltipTrigger } from '@rox-agent/ui'
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuSub,
...
import { type ThinkingLevel, THINKING_LEVELS, getThinkingLevelName } from '@rox-agent/shared/agent/thinking-levels'
...
export function FreeFormInput({
  ...
  const connectionsByProvider = React.useMemo(() => {
    ...
    for (const conn of llmConnections) {
      ...
      if (provider === 'anthropic' || provider === 'anthropic_compat' || provider === 'bedrock' || provider === 'vertex') {
        ...
        groups['Rox Agents Backend'].push(conn)
      }
    }
  ...
  // Listen for rox:insert-text events (generic mechanism for inserting text into input)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rox:insert-text', handleInsertText as EventListener)
    return () => window.removeEventListener('rox:insert-text', handleInsertText as EventListener)
  ...
  // Listen for rox:approve-plan events (used by ResponseCard's Accept Plan button)
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('rox:approve-plan', handleApprovePlan as EventListener)
    return () => window.removeEventListener('rox:approve-plan', handleApprovePlan as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])

  // Listen for rox:approve-plan-with-compact events (Accept & Compact option)
  ...
  React.useEffect(() => {
    const handleApprovePlanWithCompact = async (e: CustomEvent<PlanApprovalEventDetail>) => {
      ...
      const handleCompactionComplete = async (compactEvent: CustomEvent<{ sessionId?: string }>) => {
        ...
        window.removeEventListener('rox:compaction-complete', handleCompactionComplete as unknown as EventListener)
      ...
      window.addEventListener('rox:compaction-complete', handleCompactionComplete as unknown as EventListener)
    }

    window.addEventListener('rox:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
    return () => window.removeEventListener('rox:approve-plan-with-compact', handleApprovePlanWithCompact as unknown as EventListener)
  }, [sessionId, permissionMode, onPermissionModeChange, onSubmit, consumeInputDraftSnapshot])
  ...
  // Listen for rox:focus-input events (restore focus after popover/dropdown closes)
  React.useEffect(() => {
    ...
    window.addEventListener('rox:focus-input', handleFocusInput)
    return () => window.removeEventListener('rox:focus-input', handleFocusInput)
  ...
  // Listen for rox:paste-files events (for global paste when input not focused)
  React.useEffect(() => {
    ...
    window.addEventListener('rox:paste-files', handlePasteFiles as unknown as EventListener)
    return () => window.removeEventListener('rox:paste-files', handlePasteFiles as unknown as EventListener)
  ...
  // Listen for rox:submit-input events (simulate pressing the Send button)
  React.useEffect(() => {
    ...
    window.addEventListener('rox:submit-input', handleSubmitInput as EventListener)
    return () => window.removeEventListener('rox:submit-input', handleSubmitInput as EventListener)
