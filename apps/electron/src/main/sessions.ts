...
import { loadWorkspaceSources, loadAllSources, getSourcesBySlugs, type LoadedSource, type McpServerConfig, getSourcesNeedingAuth, getSourceCredentialManager, getSourceServerBuilder, type SourceWithCredential, isApiOAuthProvider, SERVER_BUILD_ERRORS, TokenRefreshManager, createTokenGetter } from '@rox-agent/shared/sources'
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@rox-agent/shared/config'
import { getAuthState } from '@rox-agent/shared/auth'
...
import { generateSessionTitle, regenerateSessionTitle, formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrl, getEmojiIcon, resetSummarizationClient, resolveToolIcon } from '@rox-agent/shared/utils'
...
import { DEFAULT_MODEL, getToolIconsDir } from '@rox-agent/shared/config'
...
interface ManagedSession {
  id: string
  workspace: Workspace
  agent: RoxAgent | null  // Lazy-loaded - null until first message
  messages: Message[]
...
export class SessionManager {
   ...
   */
  private async getOrCreateAgent(managed: ManagedSession): Promise<RoxAgent> {
    if (!managed.agent) {
