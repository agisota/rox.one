...
import { getToolIconsDir, isCodexModel, getMiniModel, getSummarizationModel } from '@rox-agent/shared/config'
      ...
      if (connection.authType === 'api_key' || connection.authType === 'api_key_with_endpoint' || connection.authType === 'bearer_token') {
        ...
        if (oauth?.accessToken) {
          ...
          if (isExpired && oauth.refreshToken) {
            try {
              const { refreshChatGptTokens } = await import('@rox-agent/shared/auth/chatgpt-oauth')
