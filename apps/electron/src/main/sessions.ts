...
import { RoxAgent, type AgentEvent, setPermissionMode, type PermissionMode, unregisterSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest } from '@rox-agent/shared/agent'
import {
  CodexBackend,
  CodexAgent,
  CopilotAgent,
...
} from '@rox-agent/shared/codex'
import { getLlmConnection, getDefaultLlmConnection } from '@rox-agent/shared/config'
...
import { setPathToClaudeCodeExecutable, setInterceptorPath, setExecutable } from '@rox-agent/shared/agent'
import { toolMetadataStore } from '@rox-agent/shared/network-interceptor'
...
import { formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrl, getEmojiIcon, resetSummarizationClient, resolveToolIcon } from '@rox-agent/shared/utils'
import { loadWorkspaceSkills, loadAllSkills, loadSkillBySlug, type LoadedSkill } from '@rox-agent/shared/skills'
import type { ToolDisplayMeta } from '@rox-agent/core/types'
import { getToolIconsDir, isCodexModel, getMiniModel, isAnthropicProvider, DEFAULT_MODEL, DEFAULT_CODEX_MODEL } from '@rox-agent/shared/config'
...
import { HookSystem, type HookSystemMetadataSnapshot } from '@rox-agent/shared/hooks-simple'
...
async function setupCodexSessionConfig(
  sessionPath: string,
  sources: LoadedSource[],
  mcpServerConfigs: Record<string, import('@rox-agent/shared/agent/backend').SdkMcpServerConfig>,
  sessionId?: string,
...
/** Agent type - RoxAgent for Claude, CodexBackend for Codex, CopilotAgent for Copilot */
type AgentInstance = RoxAgent | CodexBackend | CopilotAgent
...
export class SessionManager {
  ...
  setupConfigWatcher(workspaceRootPath: string, workspaceId: string): void {
    ...
    const callbacks: ConfigWatcherCallbacks = {
      ...
      onSkillChange: async (slug, skill) => {
        ...
        // Broadcast updated list to UI
        const { loadWorkspaceSkills } = await import('@rox-agent/shared/skills')
        const skills = loadWorkspaceSkills(workspaceRootPath)
        this.broadcastSkillsChanged(skills)
      },
   ...
   */
  private broadcastSkillsChanged(skills: import('@rox-agent/shared/skills').LoadedSkill[]): void {
    if (!this.windowManager) return
   ...
   * Triggered when ~/.rox-agent/permissions/default.json changes
   */
  ...
  private async runOAuthFlow(managed: ManagedSession, request: AuthRequest): Promise<void> {
    ...
    try {
      const result = await credManager.authenticate(source, {
        ...
        deeplinkScheme: process.env.ROX_DEEPLINK_SCHEME || 'roxagents',
      })
   ...
   * Creates RoxAgent for Claude or CodexBackend for Codex based on LLM connection.
   *
