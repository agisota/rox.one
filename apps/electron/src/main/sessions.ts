...
import { generateSessionTitle, regenerateSessionTitle, formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrl, getEmojiIcon, resetSummarizationClient } from '@rox-agent/shared/utils'
...
import { DEFAULT_MODEL } from '@rox-agent/shared/config'
  ...
  private processEvent(managed: ManagedSession, event: AgentEvent): void {
    ...
    switch (event.type) {
      ...
      case 'tool_start': {
        ...
        // Use parentToolUseId from the event - RoxAgent computes this correctly
        ...
        if (isParentTool) {
          ...
          // RoxAgent provided the correct parent from SDK - use it
