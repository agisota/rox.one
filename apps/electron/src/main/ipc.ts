...
import { getAuthType, setAuthType, getPreferencesPath, getCustomModel, setCustomModel, getModel, setModel, getSessionDraft, setSessionDraft, deleteSessionDraft, getAllSessionDrafts, getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace, getAnthropicBaseUrl, setAnthropicBaseUrl, loadStoredConfig, saveConfig, type Workspace, SUMMARIZATION_MODEL } from '@rox-agent/shared/config'
import { getSessionAttachmentsPath } from '@rox-agent/shared/sessions'
...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_SESSION_IN_NEW_WINDOW, async (_event, workspaceId: string, sessionId: string) => {
    ...
    const deepLink = `roxagents://allChats/chat/${sessionId}`
  ...
  ipcMain.handle(IPC_CHANNELS.SETTINGS_UPDATE_API_SETUP, async (_event, authType: AuthType, credential?: string, anthropicBaseUrl?: string | null, customModel?: string | null) => {
    ...
    if (credential) {
      if (authType === 'api_key') {
        ...
        const { getExistingClaudeCredentials } = await import('@rox-agent/shared/auth')
