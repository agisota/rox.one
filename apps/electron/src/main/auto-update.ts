 ...
 * Uses the custom manifest system at https://agents.rox.do/electron/
 *
...
import type { VersionManifest, BinaryInfo } from '@rox-agent/shared/version/manifest'
...
async function doDownloadUpdate(): Promise<void> {
  ...
  try {
    ...
    const tempDir = join(app.getPath('temp'), 'rox-agent-updates')
    ...
    const extension = process.platform === 'darwin' ? 'dmg' :
                      ...
                      'AppImage'
    installerPath = join(tempDir, `Rox-Agent-${latestVersion}.${extension}`)
...
async function installMacOS(): Promise<void> {
  ...
  const child = spawn('bash', [scriptPath, downloadedInstallerPath, app.getPath('exe')], {
    ...
    env: {
      ...process.env,
      ROX_UPDATE_DMG: downloadedInstallerPath,
      ROX_APP_PATH: app.getPath('exe'),
    },
  })
...
async function installLinux(): Promise<void> {
  ...
  const child = spawn('bash', [
    ...
    env: {
      ...process.env,
      ROX_UPDATE_APPIMAGE: downloadedInstallerPath,
      ROX_CURRENT_APPIMAGE: currentAppImage,
    },
  })
