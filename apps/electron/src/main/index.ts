...
import { cleanupSessionFileWatchForClient } from '@rox-agent/server-core/handlers/rpc'
import type { PlatformServices } from '../runtime/platform'
import type { HandlerDeps } from './handlers/handler-deps'
import type { RpcServer } from '@rox-agent/server-core/transport'
import { WsRpcServer } from '../transport/server'
import { initModelRefreshService, getModelRefreshService, setFetcherPlatform } from '@rox-agent/server-core/model-fetchers'
import { setSearchPlatform, setImageProcessor } from '@rox-agent/server-core/services'
import { createApplicationMenu } from './menu'
import { WindowManager } from './window-manager'
import { loadWindowState, saveWindowState } from './window-state'
import { getWorkspaces, loadStoredConfig, addWorkspace, saveConfig } from '@rox-agent/shared/config'
...
import { validateGitBashPath } from '@rox-agent/server-core/services'
...
// Custom URL scheme for deeplinks (e.g., roxagents://auth-complete)
// Supports multi-instance dev: ROX_DEEPLINK_SCHEME env var (roxagents1, roxagents2, etc.)
const DEEPLINK_SCHEME = process.env.ROX_DEEPLINK_SCHEME || 'roxagents'
...
if (process.env.ROX_SERVER_URL) {
  let serverOrigin: string | undefined
  try {
    serverOrigin = normalizeOriginForCert(process.env.ROX_SERVER_URL)
  } catch {
...
app.whenReady().then(async () => {
  ...
  // Seed tool icons to ~/.rox-agent/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.rox-agent/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
  ...
  try {
    ...
    const isClientOnly = !!process.env.ROX_SERVER_URL
    const isHeadless = !!process.env.ROX_HEADLESS

    if (isClientOnly) {
      mainLog.info(`Client-only mode: ROX_SERVER_URL=${process.env.ROX_SERVER_URL} (server initialization skipped)`)
    }
    ...
    if (!isClientOnly) {
      ...
      // ROX_RPC_HOST / ROX_RPC_PORT allow binding to a custom address for remote access.
      const rpcHost = process.env.ROX_RPC_HOST ?? '127.0.0.1'
      const rpcPort = process.env.ROX_RPC_PORT ? parseInt(process.env.ROX_RPC_PORT, 10) : 0
      ...
      if (isHeadless) {
        console.log(`ROX_SERVER_URL=ws://${rpcHost}:${wsServer.port}`)
        console.log(`ROX_SERVER_TOKEN=${localToken}`)
      }
