 ...
 * 2) ROX_IS_PACKAGED env (when explicitly set)
...
function resolveDebugMode(): boolean {
  ...
  const packagedEnv = process.env.ROX_IS_PACKAGED
 ...
 */
export const messagingGatewayLogPath = join(homedir(), '.rox-agent', 'logs', 'messaging-gateway.log')
