...
import { getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace } from '@rox-agent/shared/config'
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId: string, views: import('@rox-agent/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')
