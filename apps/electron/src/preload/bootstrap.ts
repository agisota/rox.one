 ...
 *
 * Thin-client mode (ROX_SERVER_URL):
...
import { createCallbackServer } from '@rox-agent/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@rox-agent/shared/auth/chatgpt-oauth-config'
import {
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.ROX_SERVER_URL!
  const wsToken = process.env.ROX_SERVER_TOKEN ?? ''
  ...
  const workspaceId = process.env.ROX_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).accountRequest = (path, init) => ipcRenderer.invoke('account:request', path, init)
