...
const api: ElectronAPI = {
  ...
  onSourcesChanged: (callback: (sources: import('@rox-agent/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, sources: import('@rox-agent/shared/sources').LoadedSource[]) => {
      callback(sources)
    }
  ...
  onSkillsChanged: (callback: (skills: import('@rox-agent/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, skills: import('@rox-agent/shared/skills').LoadedSkill[]) => {
      callback(skills)
    }
