...
const api: ElectronAPI = {
  ...
  createSource: (workspaceId: string, config: Partial<import('@rox-agent/shared/sources').FolderSourceConfig>) =>
  ...
  onSourcesChanged: (callback: (workspaceId: string, sources: import('@rox-agent/shared/sources').LoadedSource[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, workspaceId: string, sources: import('@rox-agent/shared/sources').LoadedSource[]) => {
  ...
  onSkillsChanged: (callback: (workspaceId: string, skills: import('@rox-agent/shared/skills').LoadedSkill[]) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, workspaceId: string, skills: import('@rox-agent/shared/skills').LoadedSkill[]) => {
  ...
  onAppThemeChange: (callback: (theme: import('@rox-agent/shared/config').ThemeOverrides | null) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, theme: import('@rox-agent/shared/config').ThemeOverrides | null) => {
      callback(theme)
    }
  ...
  // Menu actions (for unified Rox menu)
