...
ROX uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in ROX
...
1. **Workspace skill checked first** - If `~/.rox/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.rox/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.rox/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.rox/workspaces/{ws}/skills/commit/SKILL.md`
