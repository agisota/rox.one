...
Automations allow you to trigger actions automatically when specific events occur in ROX. You can:
...
Send a prompt to ROX (creates a new session for scheduled prompts).
...
- Environment variables are expanded (e.g., `$ROX_LABEL`)
...
{
  "type": "webhook",
  "url": "https://hooks.slack.com/services/${ROX_WH_SLACK_PATH}",
  "method": "POST",
  "body": {
    "text": "Session ${ROX_SESSION_NAME} status changed to ${ROX_NEW_STATE}"
  }
}
```
...
```json
{
  ...
  "auth": {
    "type": "bearer",
    "token": "${ROX_WH_API_TOKEN}"
  },
  "body": { "event": "$ROX_EVENT" }
}
```
...
```json
{
  ...
  "auth": {
    "type": "basic",
    "username": "${ROX_WH_USER}",
    "password": "${ROX_WH_PASS}"
  }
}
```
...
**Security:** Webhook actions only have access to `ROX_*` system variables and `ROX_WH_*` user-defined secrets. They do **not** have access to your full system environment (e.g., `$HOME`, `$PATH`, or other process variables).

## Environment Variables

Both prompt and webhook actions support variable expansion using `$VAR` or `${VAR}` syntax.

### System Variables (ROX_*)

These are automatically set by the automation system based on the triggering event:

| Variable | Description | Available For |
|----------|-------------|---------------|
| `$ROX_EVENT` | Event name (e.g., `LabelAdd`) | All events |
| `$ROX_EVENT_DATA` | Full event payload as JSON | All events |
| `$ROX_SESSION_ID` | Session ID | Events with session context |
| `$ROX_SESSION_NAME` | Session name | Events with session context |
| `$ROX_WORKSPACE_ID` | Workspace ID | All events |

**Per-event variables:**

| Event | Variable | Description |
|-------|----------|-------------|
| `LabelAdd` / `LabelRemove` | `$ROX_LABEL` | The label that was added/removed |
| `PermissionModeChange` | `$ROX_OLD_MODE`, `$ROX_NEW_MODE` | Previous and new permission mode |
| `FlagChange` | `$ROX_IS_FLAGGED` | `true` or `false` |
| `SessionStatusChange` | `$ROX_OLD_STATE`, `$ROX_NEW_STATE` | Previous and new status |
| `SchedulerTick` | `$ROX_LOCAL_TIME`, `$ROX_LOCAL_DATE` | Current time (`14:30`) and date (`2026-03-09`) |

### User-Defined Webhook Secrets (ROX_WH_*)

For webhook actions, you can define your own secrets by setting environment variables with the `ROX_WH_` prefix in your shell profile (e.g., `~/.zshrc`, `~/.bashrc`):

```bash
# In your shell profile
export ROX_WH_SLACK_URL="https://hooks.slack.com/services/T.../B.../xxx"
export ROX_WH_DISCORD_URL="https://discord.com/api/webhooks/123/abc"
export ROX_WH_API_TOKEN="your-secret-token"
```
...
{
  "type": "webhook",
  "url": "${ROX_WH_SLACK_URL}",
  "method": "POST",
  "body": { "text": "Hello from ROX!" }
}
```
...
```json
{
  "type": "webhook",
  "url": "https://api.example.com/events",
  "headers": { "Authorization": "Bearer ${ROX_WH_API_TOKEN}" },
  "body": { "event": "${ROX_EVENT}", "session": "${ROX_SESSION_NAME}" }
}
```

This keeps secrets out of `automations.json` (which may be shared or committed to version control).

> **Note:** Only variables prefixed with `ROX_WH_` are injected into webhook actions. Other environment variables (like `$HOME` or `$DATABASE_URL`) are not accessible to webhooks.
...
4. **Pair the supergroup.** In Rox Agent: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.rox-agent/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
        ...
        "actions": [
          {
            ...
            "url": "${ROX_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":warning: Permission escalated from safe to allow-all in *${ROX_SESSION_NAME}*" }
          }
        ]
      }
    ]
...
```json
{
  ...
  "automations": {
    "LabelAdd": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ROX_LABEL was added. Log this change with a timestamp." }
        ]
      }
    ],
    "LabelRemove": [
      {
        "actions": [
          { "type": "prompt", "prompt": "The label $ROX_LABEL was removed. Log this change with a timestamp." }
        ]
      }
    ]
  }
...
Sends a Slack message when a session is marked as done. Requires `ROX_WH_SLACK_URL` in your shell profile.
        ...
        "actions": [
          {
            ...
            "method": "POST",
            "body": {
              "text": ":white_check_mark: Session *${ROX_SESSION_NAME}* marked as done"
            }
          }
        ]
      }
        ...
        "actions": [
          {
            ...
            "url": "${ROX_WH_SLACK_URL}",
            "method": "POST",
            "body": { "text": ":rotating_light: Urgent label added to *${ROX_SESSION_NAME}*" }
          },
          {
            "type": "prompt",
        ...
        "actions": [
          {
            ...
            "body": {
              ...
              "client_id": "${ROX_WH_CLIENT_ID}",
              "client_secret": "${ROX_WH_CLIENT_SECRET}"
            }
          }
        ]
      }
    ...
    "SessionStatusChange": [
      {
        ...
        "actions": [
          {
            "type": "webhook",
            "url": "https://api.example.com/rox-events",
            "method": "POST",
            "headers": {
              "Authorization": "Bearer ${ROX_WH_API_TOKEN}",
              "X-Source": "rox-agent"
            },
            "body": {
              "event": "${ROX_EVENT}",
              "session_id": "${ROX_SESSION_ID}",
              "old_status": "${ROX_OLD_STATE}",
              "new_status": "${ROX_NEW_STATE}"
            }
          }
        ]
      }
...
Ask ROX to validate your automations configuration:
...
2. **Check env vars** — Ensure `ROX_WH_*` variables are set in your shell profile and ROX was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
...
5. **Keep secrets out of config** - Use `ROX_WH_*` env vars for webhook URLs and tokens instead of hardcoding them in automations.json
