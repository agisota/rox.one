...
This guide explains how to configure hooks in Rox Agent to automate workflows based on events.

## What Are Hooks?

Hooks allow you to trigger actions automatically when specific events occur in Rox Agent. You can:
...
```
~/.rox-agent/workspaces/{workspaceId}/hooks.json
```
...
- `ROX_EVENT` - Event name (e.g., "LabelAdd")
- `ROX_EVENT_DATA` - Full event data as JSON
- `ROX_SESSION_ID` - Current session ID
- `ROX_WORKSPACE_ID` - Current workspace ID
- `ROX_WORKING_DIR` - Current working directory
- Event-specific variables (e.g., `ROX_LABEL` for label events)
    ...
    "LabelAdd": [
      {
        "permissionMode": "allow-all",
        "hooks": [
          { "type": "command", "command": "echo \"[$(date)] Added: $ROX_LABEL\" >> ~/label-log.txt" }
        ]
      }
    ],
    "LabelRemove": [
      {
        "permissionMode": "allow-all",
        "hooks": [
          { "type": "command", "command": "echo \"[$(date)] Removed: $ROX_LABEL\" >> ~/label-log.txt" }
        ]
      }
    ]
  }
    ...
    "LabelAdd": [
      {
        ...
        "permissionMode": "allow-all",
        "hooks": [
          { "type": "command", "command": "osascript -e 'display notification \"Urgent session flagged\" with title \"Rox Agent\"'" }
        ]
      }
    ]
  }
...
Ask Rox Agent to validate your hooks configuration:
