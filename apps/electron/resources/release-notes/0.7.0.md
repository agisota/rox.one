# v0.7.0 — Rox CLI, Headless Server & Architecture Overhaul
...
- **`rox-cli run`** — launch agent sessions directly from your terminal with prompt input and streaming output
...
### Why it's cool
This is the first step toward full programmatic control of Rox Agents. Script your agent workflows, integrate into CI/CD pipelines, or run sessions headlessly on remote machines. Partially addresses [#343](https://github.com/lukilabs/rox-agents-oss/issues/343).

## Headless Server

Run Rox Agents as a standalone server without the Electron desktop app — deploy on any machine, connect from any client.
...
### Why it's cool
Deploy Rox Agents on a remote server and connect from anywhere. Run it on your home lab, a cloud VM, or a CI runner. The thin-client mode means the browser UI stays lightweight while all the heavy lifting happens server-side. Partially addresses [#291](https://github.com/lukilabs/rox-agents-oss/issues/291).
...
- **Server-core extraction** — 14 core handlers, session management, services, and domain utilities extracted into `@rox-agent/server-core` for reuse across Electron and headless
- **Protocol formalization** — channels, DTOs, and event maps extracted to `@rox-agent/shared/protocol` with wire-format stability tests
...
- **Pi 3-tier model persistence** — fixed model save/hydration for Best/Balanced/Fast tiers. Fixes [#336](https://github.com/lukilabs/rox-agents-oss/issues/336)
- **Multi-panel input focus** — fixed focus scoping and race handling that caused keystrokes to jump between panels. Fixes [#339](https://github.com/lukilabs/rox-agents-oss/issues/339)
