  ...
  return [
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        const tmpDir = await mkdtemp(`${tmpdir()}/rox-validate-`)
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        if (ctx.baseUrl) {
          ...
          let key = ''
          try {
            key = resolveApiKey(provider, ctx.apiKey || '', ctx.baseUrl)
          } catch (error) {
    ...
    {
      name: 'mcp:rox-public (auth:none)',
      fn: async (client, ctx) => {
        if (!ctx.createdSessionId) return 'skipped (no session)'
        // Enable the pre-committed rox-public MCP source on the session
        const enableSlugs = [ctx.createdSourceSlug, 'rox-public'].filter(Boolean) as string[]
        await client.invoke('sessions:command', ctx.createdSessionId, {
          type: 'setSources',
          sourceSlugs: enableSlugs,
        })
        return await waitForSendEvents(client, ctx.createdSessionId,
          `[source:rox-public] List the documents under the "RoxAgents E2E Test" folder inside the "RoxAgents" folder. Just list their names.`,
    ...
    },
    {
      ...
      fn: async (client, ctx) => {
        ...
        const enableSlugs = [ctx.createdSourceSlug, 'rox-public', 'stitch-mcp'].filter(Boolean) as string[]
        await client.invoke('sessions:command', ctx.createdSessionId, {
          type: 'setSources',
          sourceSlugs: enableSlugs,
        })
...
description: "Validation skill created by rox-cli"
requiredSources:
  - "${sourceSlug}"
---
...
Examples:
  rox-cli run "What files are in the current directory?"
  rox-cli run --source rox-kb "Summarize today's daily note"
  rox-cli run --workspace-dir .github/agents --source rox-public "Read the doc"
  rox-cli run --provider openai --model gpt-4o "Summarize this repo"
  OPENAI_API_KEY=sk-... rox-cli run --provider openai "Hello"
  GOOGLE_API_KEY=... rox-cli run --provider google --model gemini-2.0-flash "Hello"
  DEEPSEEK_API_KEY=sk-... rox-cli run --provider deepseek --model deepseek-v4-flash "Hello"
  echo "Analyze this code" | rox-cli run
  rox-cli ping
  rox-cli sessions
