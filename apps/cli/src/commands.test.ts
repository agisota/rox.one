...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    const prevUrl = process.env.ROX_SERVER_URL
    const prevToken = process.env.ROX_SERVER_TOKEN
    const prevCa = process.env.ROX_TLS_CA

    process.env.ROX_SERVER_URL = 'ws://env-server:8080'
    process.env.ROX_SERVER_TOKEN = 'env-token'
    process.env.ROX_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.ROX_SERVER_URL
      else process.env.ROX_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.ROX_SERVER_TOKEN
      else process.env.ROX_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.ROX_TLS_CA
      else process.env.ROX_TLS_CA = prevCa
    }
  })
  ...
  it('--source accumulates into array', () => {
    ...
    expect(args.sources).toEqual(['rox-kb', 'github'])
  })
