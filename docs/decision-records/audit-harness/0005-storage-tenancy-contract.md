...
- **Default parameter keeps every commit shippable.** The 28 known importers of `'@rox-agent/shared/config/storage'` continue to compile unchanged after the signature widening. Migration of call sites to pass `DEFAULT_LOCAL_SCOPE` explicitly is mechanical and incremental — no big-bang flip.
