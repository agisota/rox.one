...
- **Pi's dead `pendingToolExecutions` map.** This map is only ever iterated for bulk-reject in `pi-agent.ts`; the real symmetric map lives in `pi-agent-server/src/index.ts`. The migration kept it (using `PendingRequestMap`) because removing it would change behavior on subprocess teardown if a future code path starts populating it. Tracked as a cleanup item for the next slice.
