...
- Removed `default-workspace-bundle.ts` from `packages/shared/src/workbench/index.ts` so renderer imports from `@rox-agent/shared/workbench` do not pull `fs`, `path`, label storage, or status storage into the browser bundle.
...
- Electron build now proves renderer imports from `@rox-agent/shared/workbench` no longer pull `fs`/`path` through the Node-only default workspace bundle.
