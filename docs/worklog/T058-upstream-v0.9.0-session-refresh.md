...
Ported the safe subset of upstream `lukilabs/rox-agents-oss` v0.9.0 session-list recovery into the ROX ONE fork: stale reconnect metadata refresh no longer deletes locally-visible sessions when `getSessions()` returns a transient partial list.
...
- Local repo: `/Users/marklindgreen/Projects/rox/rox`, branch `main`, HEAD before edits `b36b00c`.
