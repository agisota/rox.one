...
Renamed active non-UI Rox-coded identifiers and files to canonical ROX names
while preserving the one-minor `RoxAgentConfig` compatibility alias.
...
`RoxAgentConfig` type alias in `packages/shared/src/agent/claude-agent.ts`.
...
- `packages/pi-agent-server/src/rox-metadata-schema.test.ts`
...
- legacy `rox-metadata-schema.ts` and `sync-rox-agent-bash-patterns.ts`
  files are absent and canonical replacements exist;
- active source roots contain no legacy non-UI identifiers such as
  `RoxMcpClient`, `RoxOAuth`, `allowRoxMetadataProperties`, or
  `getRoxAgentReadOnlyBashPatterns`;
- `RoxAgentConfig` remains only as a deprecated alias with explicit
...
- Renamed `RoxMcpClient` to `RoxMcpClient` and updated shared/server-core
  callers.
- Renamed `RoxOAuth` to `RoxOAuth` and updated credential-manager callers.
...
- Kept `RoxAgentConfig` as the only compatibility alias with explicit
...
- `bun test packages/shared/tests/permissions-rox-agent-sync.test.ts`
...
The `rox-agent` CLI command strings and package names are intentionally
...
| Deprecated `RoxAgentConfig` alias exists | Green | Regression test checks alias plus `remove in v1.1.0` JSDoc |
