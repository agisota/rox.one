...
- Updated `PiAgent.spawnSubprocess()` to generate a per-spawn UUID integrity
  token, pass it to the child through `CRAFT_PI_SCOPE_IPC_TOKEN`, and include
...
| Crafted cross-workspace envelopes reject | Pass | Forgery test throws `MultiTenantForgeryError` |
