...
- Rewrote README Docker example image/volume names to ROX.ONE naming without
  changing the still-owned-later `ROX_*` environment variable examples.
...
- Added a README `Acknowledgements` section that preserves upstream
  attribution for `https://github.com/lukilabs/rox-agents-oss`.
...
- `git diff --check`
- `rg -n "rox-cli|rox-agent/|rox-agents-oss|@rox-agent" README.md CONTRIBUTING.md`
