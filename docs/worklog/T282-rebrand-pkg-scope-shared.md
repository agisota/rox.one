...
Rename the shared workspace package scope from `@rox-agent/shared` to
`@rox-one/shared`.
...
- Expected failure: shared package metadata still reported
  `@rox-agent/shared` where the new test expected `@rox-one/shared`.

## 6. Implementation changes

- Renamed `packages/shared/package.json` from `@rox-agent/shared` to
  `@rox-one/shared`.
- Replaced active `@rox-agent/shared` workspace dependencies in app and
...
- Did not add an ad hoc `@rox-agent/shared` compatibility shim in this
...
- `git diff --check`
- `rg -n "@rox-agent/shared" apps packages scripts package.json tsconfig*.json bun.lock --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omx/**' --glob '!**/.omc/**'`
- `bun run typecheck`
- `bun run lint`
...
- Targeted active grep for `@rox-agent/shared`: exit 1 with no matches.
...
| Red test proves shared scope gap | Pass | Targeted package-scope suite failed on `@rox-agent/shared` metadata before implementation |
| Shared package metadata uses ROX scope | Pass | `packages/shared/package.json` now names `@rox-one/shared` |
| Active shared dependencies, imports, paths, and aliases use ROX scope | Pass | Targeted active grep finds no `@rox-agent/shared`; R.5 test passes |
