...
- Expected failure: `packages/test-fixtures/package.json` still reported
  `@craft-agent/test-fixtures` while the test expected `@rox-one/test-fixtures`.
...
- `rg -n "@craft-agent/test-fixtures|@rox-one/test-fixtures" packages/test-fixtures packages/shared/package.json packages/shared/tests/mode-manager.test.ts packages/shared/tests/mode-manager-bash-validation.test.ts bun.lock`
...
- `bun run validate:rebrand`: expected exit 1 with 3921 remaining forbidden
  ...
  package/env/CLI surfaces (`craft-agent`, `@craft-agent`, `CRAFT_`,
  `craft-cli`).
