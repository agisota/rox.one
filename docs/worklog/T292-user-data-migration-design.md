...
- `packages/shared/src/config/paths.ts` already contains a *partial*
  auto-migration block: when `~/.rox/` is missing and `~/.rox-agent/`
...
- The Phase R.8 spec requires a marker file (`.migrated-from-rox`),
...
- Goals & non-goals.
- Detection priority (`~/.rox-agent/` before `~/.rox/`).
