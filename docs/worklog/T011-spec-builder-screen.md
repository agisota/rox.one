...
- Rewrite and Thinking Partner flows already emit `rox:spec-builder-intent` payloads, but the app has no unified `spec-builder` route/listener yet.
...
## 10. Remaining risks
- The screen is implemented as a standalone renderer component; route registration and the central `rox:spec-builder-intent` listener are intentionally deferred to the next integration slice.
