...
`rox*` names to canonical `rox*` names while preserving launcher behavior.

## 2. Repo context discovered

- Phase R.3 requires `apps/electron/resources/bin/rox-agent` and
  `rox-agent.cmd` to become `rox-agent` and `rox-agent.cmd`.
- The shell and cmd wrappers dispatch through existing `ROX_*` env vars;
...
- `apps/electron/resources/docs/rox-cli.md` is the bundled CLI command
  reference and contains `rox-agent` command examples throughout the body.
...
- `apps/electron/resources/tool-icons/tool-icons.json` maps the bundled ROX
  CLI entry to `rox-agent.png` and the `rox-agent` command.
- The app resource docs link to `rox-cli.md` from labels, sources, skills,
...
- legacy `apps/electron/resources/tool-icons/rox-agent.png` is absent and
  `apps/electron/resources/tool-icons/rox-agent.png` exists;
- legacy `apps/electron/resources/docs/rox-cli.md` is absent and
  `apps/electron/resources/docs/rox-cli.md` exists;
- legacy `apps/electron/resources/bin/rox-agent` and `.cmd` are absent and
...
- Updated packaged and debug `ROX_COMMANDS_DOC_PATH` values in
  `apps/electron/src/main/index.ts` to point at `rox-cli.md`.
- Renamed the shared doc ref from `DOC_REFS.roxCli` to `DOC_REFS.roxCli`
...
- `bun run build`
- `git diff --check`
- `git ls-files | rg 'apps/electron/resources/(tool-icons/rox-agent\\.png|docs/rox-cli\\.md|bin/rox-agent(\\.cmd)?$)'`
- `rg -n "rox-cli\\.md|rox-agent\\.png|apps/electron/resources/bin/rox-agent|DOC_REFS\\.roxCli|roxCli:" apps/electron/resources apps/electron/src/main packages/shared/src scripts/__tests__/rebrand-asset-paths.test.ts`
...
- Phase R.6 owns `ROX_*` env-var migration, so T268 intentionally leaves
