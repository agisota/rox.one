...
`@craft-agent/` package-scope references remain in TypeScript, package metadata,
or tsconfig files.
...
- Updated generated server build workspace symlink creation from
  `node_modules/@craft-agent` to `node_modules/@rox-one`.
...
- `bun test scripts/__tests__/rebrand-package-scope.test.ts`
- `rg -n "@craft-agent/" scripts apps packages package.json tsconfig*.json --glob '*.ts' --glob '*.tsx' --glob 'package.json' --glob 'tsconfig*.json' --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omx/**' --glob '!**/.omc/**'`
- `bun run typecheck`
- `bun run lint`
...
- The shared/server-core transitional package-shim requirement still needs a
  ...
  active `@craft-agent/` package metadata or TypeScript literals without that
  design decision.
