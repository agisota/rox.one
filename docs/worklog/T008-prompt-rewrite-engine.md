...
- T007 toolbar emitted `craft:product-mode-intent` but had no runtime consumer yet.
- `ProductModeRegistry` already declares `rewrite` with `prompt-rewriter-pack`, prompt artifacts, and validation gates.
- Renderer build must not import `@craft-agent/shared/workbench` index from browser code because the index also exports Node-only workspace bundle code that imports `fs/path`.
...
- Wired `FreeFormInput` so `rewrite-prompt` still dispatches `craft:product-mode-intent` and also opens the local rewrite modal.
- Added all `workbench.rewrite.*` locale keys to `de`, `en`, `es`, `hu`, `ja`, `pl`, `ru`, and `zh-Hans`.
- Fixed renderer import to use `@craft-agent/shared/workbench/prompt-rewrite-engine` instead of the shared workbench index, avoiding Node-only exports in the browser bundle.
