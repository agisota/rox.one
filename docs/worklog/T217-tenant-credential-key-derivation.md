...
- Current encryption uses AES-256-GCM with a `CRAFT01\0` header and PBKDF2 key
  derived from a stable machine id plus `craft-agent-v2`.
- A legacy PBKDF2 key using hostname, username, homedir, and `craft-agent-v1`
...
- `DEFAULT_LOCAL_SCOPE` did not write under the test `CRAFT_CONFIG_DIR`
...
- The red test run exposed that the pre-existing backend ignored
  `CRAFT_CONFIG_DIR`; the implementation now routes through `getConfigDir()`,
