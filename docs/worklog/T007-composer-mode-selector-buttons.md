...
The buttons do not execute backend workflows yet. They emit a typed renderer event payload via `craft:product-mode-intent` so later tasks can attach rewrite/spec/review/TDD execution without changing the composer submit path.
...
- Added `@craft-agent/shared/workbench` and `@craft-agent/shared/workbench/product-mode-registry` exports for future tasks.
