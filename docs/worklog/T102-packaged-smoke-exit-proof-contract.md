...
0 pass
1 fail
Expected to not contain: "'CRAFT_SERVER_URL=': false"
```
...
- stale `CRAFT_SERVER_URL=` marker state is absent;
- URL/token redaction remains;
- normal `scripts/electron-smoke.ts` still requires `CRAFT_SERVER_URL=` and
...
- Removed the stale `CRAFT_SERVER_URL=` entry from the packaged smoke `seen`
  map.
...
|---|---|---|
| `bun test scripts/__tests__/electron-packaged-smoke-contract.test.ts` | RED, expected | 0 pass, 1 fail on stale `CRAFT_SERVER_URL=` seen state |
