...
- `bun run electron:smoke`: passed outside the Codex sandbox with `CRAFT_SERVER_URL=` and `App initialized successfully` markers. The in-sandbox attempt aborted before app code with Electron `SIGABRT`; verifying `electron --version` outside sandbox confirmed this was a sandbox/GUI launch constraint, not a runtime regression.
