...
Rename the server workspace package scopes from `@craft-agent/server` and
`@craft-agent/server-core` to `@rox-one/server` and
`@rox-one/server-core`.

## 2. Repo context discovered

- Phase R.5.9 follows the landed Pi agent server package scope rename.
- Direct `rg` found `@craft-agent/server` in `packages/server/package.json`
  and `bun.lock`.
- Direct `rg` found `@craft-agent/server-core` across package metadata,
  workspace dependencies, tsconfig paths, source imports, tests, comments,
  package README, `scripts/build-server.ts`, and `bun.lock`.
- `@craft-agent/shared` remains intentionally untouched for R.5.10.
...
- active `apps/`, `packages/`, `scripts/build-server.ts`, and `bun.lock` files
  no longer contain exact package references to `@craft-agent/server` or
...
- Expected failure: `packages/server/package.json` still reported
  `@craft-agent/server` while the test expected `@rox-one/server`.
...
- Renamed `packages/server-core/package.json` package name to
  `@rox-one/server-core`.
- Updated active `@craft-agent/server-core` workspace dependencies, imports,
...
- Updated the server package comment and `bun.lock` entries from
  `@craft-agent/server` to `@rox-one/server`.
...
- Targeted active-scope grep:
  `rg -n "@craft-agent/server-core|@craft-agent/server(?!-core)" apps packages scripts bun.lock package.json tsconfig*.json --pcre2 --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omx/**' --glob '!**/.omc/**'`
...
- The R.5.10 `@craft-agent/shared` rename remains intentionally untouched, so
  active imports from `@craft-agent/shared` continue to exist until the next
  sub-phase.
- The package-scope compatibility shim requirement covers `server-core` and
  ...
  prove it without reintroducing active `@craft-agent/server-core` matches.
