...
`@craft-agent/session-mcp-server` to `@rox-one/session-mcp-server`.
...
- Expected failure: `packages/session-mcp-server/package.json` still reported
  `@craft-agent/session-mcp-server` while the test expected
...
- `rg -n "@craft-agent/session-mcp-server|@rox-one/session-mcp-server" apps packages bun.lock tsconfig*.json package.json --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omc/**' --glob '!**/.omx/**'`
