...
- The test harness registers `registerWorkspaceCoreHandlers()` against a synthetic `RpcServer`, writes flat and tenant fixture config under a temp `CRAFT_CONFIG_DIR`, and invokes `RPC_CHANNELS.workspaces.GET` directly.
