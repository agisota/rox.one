...
Rename the shared UI workspace package scope from `@rox-agent/ui` to
`@rox-one/ui`.
...
- Expected failure: `packages/ui/package.json` still reported
  `@rox-agent/ui` while the test expected `@rox-one/ui`.
...
- `rg -n "@rox-agent/ui|@rox-one/ui" apps packages bun.lock --glob '!**/dist/**' --glob '!**/node_modules/**'`
