...
2. **Case 2: legacy only** — Seeds `~/.rox-agent/` fixture with five
   ...
   tree is left intact (copy not move), that the `.migrated-from-rox`
...
5. **Priority order** — Seeds both `~/.rox-agent/` and `~/.rox/`
   with DIFFERENT contents. Asserts the copy source is
   `~/.rox-agent/` (per spec §3) and that `~/.rox/` is left
   untouched.
...
2. Legacy root detection: iterates `legacyRoots` (default
   `[~/.rox-agent, ~/.rox]`) and picks the first one that exists.
...
- **Pre-Node-22 hosts**: `verbatimSymlinks: true` is silently ignored
  ...
  `~/.rox-agent/`); flagged here for completeness.
