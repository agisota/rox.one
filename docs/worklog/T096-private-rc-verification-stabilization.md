...
```bash
HOME=/private/tmp/rox-bun-test-home bun test packages/shared/src/__tests__/unified-network-interceptor.schema.test.ts
```
...
- `packages/shared/src/interceptor-common.ts` resolved interceptor config through
  `homedir()` and ignored the existing `ROX_CONFIG_DIR` override used by the
  config subsystem.
...
- Electron itself was able to start; the `SIGABRT` reproduced only inside the
  ...
  `ROX_SERVER_URL=`, `App initialized successfully`, and the smoke pass marker.

## 4. Implementation changes

- Routed interceptor config/error fallback paths through `getConfigDir()` so
  `ROX_CONFIG_DIR` controls test and subprocess config lookup.
- Reworked the unified network interceptor schema test to create a temp
  `ROX_CONFIG_DIR`, clean only that temp directory, and assert it does not use
