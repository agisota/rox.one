...
`Rox*` names to canonical `Rox*` names without changing rendered output.

## 2. Repo context discovered

- Explorer mapping found three UI icon component definitions:
  `RoxAppIcon`, `RoxAgentsLogo`, and `RoxAgentsSymbol`.
- `RoxAppIcon` had no current consumers outside its own file.
- `RoxAgentsLogo` was consumed by the playground icon registry.
- `RoxAgentsSymbol` was consumed by `AppMenu`, `TopBar`, `SplashScreen`,
...
- active Electron renderer source has no `RoxAppIcon`,
  `RoxAgentsLogo`, or `RoxAgentsSymbol` identifiers.
...
- Expected failure: `legacy icon component files should be renamed`, with the
  received list containing `RoxAppIcon.tsx`, `RoxAgentsLogo.tsx`, and
  `RoxAgentsSymbol.tsx`.

## 6. Implementation changes

- Renamed:
  - `RoxAppIcon.tsx` -> `RoxAppIcon.tsx`
  - `RoxAgentsLogo.tsx` -> `RoxAgentsLogo.tsx`
  - `RoxAgentsSymbol.tsx` -> `RoxAgentsSymbol.tsx`
...
- Registry IDs such as `rox-agents-logo` remain lower-case product tokens and
  are outside the T264 representative `Rox*` component identifier scope; later
