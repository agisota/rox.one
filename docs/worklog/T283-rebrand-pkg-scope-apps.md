...
Rename the four app workspace package scopes from `@craft-agent/<app>` to
`@rox-one/<app>`.
...
- Generic bare `craft-agent` tokens remain out of scope for this package-name
  ticket.
...
- Expected failure: app package metadata still reported `@craft-agent/cli`
...
- Left generic bare `craft-agent` ESLint plugin ids, CLI command names, env
...
- `git diff --check`
- `rg -n "@craft-agent/(cli|electron|viewer|webui)" apps packages scripts package.json tsconfig*.json bun.lock --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omx/**' --glob '!**/.omc/**'`
- `bun run typecheck`
- `bun run lint`
...
- Root package name, CLI binary names, `CRAFT_*` env vars, config dirs, and
  historical docs still carry legacy tokens until later rebrand phases.
- The R.5 closeout still needs to assert zero active `@craft-agent/` package
...
| Red test proves app scope gap | Pass | Targeted package-scope suite failed on `@craft-agent/cli` metadata before implementation |
