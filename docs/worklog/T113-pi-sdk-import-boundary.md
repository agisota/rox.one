...
server/runtime imports of `@craft-agent/shared/config` do not transitively load
...
- the shared package exposes `@craft-agent/shared/config/models-pi`;
...
Expected to contain: "from '@craft-agent/shared/config/models-pi'"
...
- Added the explicit package export
  `@craft-agent/shared/config/models-pi`.
...
- Updated server-core PI provider discovery handlers to:
  ...
  - dynamically import only `@craft-agent/shared/config/models-pi` after the
    guard passes.
...
| `@craft-agent/shared/config` no longer exports `models-pi` | Done | Static contract plus config barrel diff |
| `@craft-agent/shared/config/models-pi` is exported explicitly | Done | Static contract plus `packages/shared/package.json` export |
