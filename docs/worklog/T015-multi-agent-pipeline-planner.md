...
Supervisor integration initially caught a renderer build failure after this worker slice because Spec Builder imported the broad `@rox-agent/shared/automations` barrel, which pulled `resolve-config-path.ts` and `node:crypto` into the browser bundle.

The fix added the browser-safe `@rox-agent/shared/automations/presets` package export and changed the renderer to import only that preset surface. `bun run electron:build` then completed successfully, and `bun run electron:smoke` reached `[smoke] Electron headless startup passed`.
