...
| R.6 | Env-var shim | `777ada7` (#66) | T285, T286, T287, T288 | `readEnv()` shim lands; legacy `ROX_*` vars continue to work for one minor; deprecation warning coverage added. |
...
| R.8 | User-data migration | `3f9ea58`, `efdf1bc`, `f39d087`, merge `f427768` (#68) | T292, T293, T294 | `migrateUserDataIfNeeded()` shim ports `~/.rox-agent/` to `~/.rox/` on first Electron startup. |
