...
- Direct `rg` found `@craft-agent/messaging-gateway` and
  `@craft-agent/messaging-whatsapp-worker` in package metadata, runtime
...
- Expected failure: `packages/messaging-gateway/package.json` still reported
  `@craft-agent/messaging-gateway` while the test expected
