...
| No open PRs | `gh pr list --state open --json number,title,headRefName,url --limit 200` reports 0 open PRs; PR #216 merged into `origin/main` as `0b0a218f`, and PR #214 closed without merge | Green |
...
| Remote branches reviewed | Explicit pre-rewrite helper requires origin to expose only `main` and `backup/pre-rebrand-history-rewrite-2026-05-13`; origin currently has 150 non-main/non-R.11-backup branches | Pre-rewrite blocked |
...
- GitHub reports 0 open PRs against `main`. PR #216 merged into `origin/main`
  as `0b0a218f`, and PR #214 closed without merge; both remote heads still
  remain in the operator-owned remote branch review queue.
...
- Origin currently has 150 non-main/non-R.11-backup branches, so the explicit
...
- The helper now checks stale remote branch state in explicit pre-rewrite mode;
  the latest run reports 150 branches outside `main` and the R.11 backup
  branch.
...
tag-on-main, backup artifacts, stale remote branches, and fork review:
...
remote-branch-review fail    origin has 150 non-main/non-R.11-ba...
fork-review          fail    GitHub reports 1 fork(s); expected 0.
no-open-prs          pass    0 open PRs.
current-branch       pass    Current checkout is main.
main-sync            pass    origin/main...main is 0 0.
...
| --- | --- | --- |
| R.11 preflight is green before backup creation | Blocked | Default pre-backup gate fails on active goal, `fork-review`, `rebrand-tag-local-sync`, and `rebrand-tag-on-main`; `no-open-prs` is green |
...
| Remote branches reviewed before rewrite | Blocked | Explicit pre-rewrite gate fails on 150 non-main/non-R.11-backup origin branches |
