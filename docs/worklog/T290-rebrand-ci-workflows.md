...
contributors cannot reintroduce `Craft` or `craft-agent-` in those
...
The legacy `CRAFT_*` env-var keys (`CRAFT_E2E_FAKE_PROVIDERS`,
`CRAFT_HEADLESS`, `CRAFT_DEV_RUNTIME`, `secrets.CRAFT_ANTHROPIC_API_KEY`)
...
lines, and asserts the value contains neither `craft-agent` nor (case-
insensitive) `Craft\b`. The second scans for any `craft-agent-` substring
...
workflow edit that reintroduces `Craft` or `craft-agent-` will fail
...
- The R.10 closeout will need to retire the CRAFT_* env-var keys in
...
- The `secrets.CRAFT_ANTHROPIC_API_KEY` reference depends on an org-
...
- [x] CRAFT_* env-var keys remain in workflow YAML to exercise the
      R.6 shim.
