...
```text
ROX_SERVER_URL=[REDACTED]
ROX_SERVER_TOKEN=[REDACTED]
...
- `bun run electron:smoke:packaged:mac`: passed after launching packaged `ROX ONE.app`, redacting `ROX_SERVER_URL` and `ROX_SERVER_TOKEN`, and printing `ROX ONE packaged headless startup passed`.
