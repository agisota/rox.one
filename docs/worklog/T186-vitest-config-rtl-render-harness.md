...
- `@radix-ui/react-tooltip` (inside `@craft-agent/ui`) requires a `TooltipProvider` ancestor. Without it, every render of a Tooltip-bearing composer component throws during the context lookup. `TooltipProvider` is therefore mandatory in `TestComposerHarness`.
...
- `node_modules/@craft-agent/ui/` — TooltipProvider export confirmed
...
- `resolve.dedupe: ['react', 'react-dom']` pins both packages to the workspace root copy. Without this `@craft-agent/ui`'s peer dep resolution pulls in a second React copy, breaking hooks invariants at runtime.
