...
Rename the core workspace package scope from `@rox-agent/core` to
`@rox-one/core`.
...
- Expected failure: `packages/core/package.json` still reported
  `@rox-agent/core` while the test expected `@rox-one/core`.
...
- Updated active imports, type imports, tsconfig path mappings, package docs,
  and package comments from `@rox-agent/core` to `@rox-one/core`.
...
- `rg -n "@rox-agent/core|@rox-one/core" apps packages bun.lock tsconfig*.json package.json --glob '!**/dist/**' --glob '!**/node_modules/**' --glob '!**/.omc/**' --glob '!**/.omx/**'`
...
- Post-commit `bun run validate:rebrand`: expected exit 1 with 3423 remaining
  ...
  later-phase package/env/CLI surfaces (`rox-agent`, `@rox-agent`,
  `ROX_`, `rox-cli`).
