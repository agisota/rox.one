...
`permissions-craft-agent-sync.test.ts` and
`permissions-config-craft-cli-flag.test.ts`. No snapshot or fixture files were
...
- Renamed `packages/shared/tests/permissions-craft-agent-sync.test.ts` to
  `packages/shared/tests/permissions-agent-sync.test.ts`.
- Renamed
  `packages/shared/src/agent/__tests__/permissions-config-craft-cli-flag.test.ts`
  to `packages/shared/src/agent/__tests__/permissions-config-cli-flag.test.ts`.
- Left `craft-agent` command strings and `CRAFT_FEATURE_CRAFT_AGENTS_CLI`
...
The renamed tests still contain `craft-agent` command strings and
`CRAFT_FEATURE_CRAFT_AGENTS_CLI` because later R.2/R.6 phases own those runtime
compatibility names.
