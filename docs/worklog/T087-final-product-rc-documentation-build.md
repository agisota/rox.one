...
- `docs/release/v1-migration-guide.md` — 211 LOC. Config dir migration via
  R.8 shim, env-var rename table (`CRAFT_*` → `ROX_*`) with shim window,
  package-scope rename `@craft-agent/*` → `@rox-one/*`, Docker image
  rename `craft-agent-server` → `rox-one-server`, CLI binary alias
