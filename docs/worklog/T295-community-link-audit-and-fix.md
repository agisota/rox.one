...
repo/issues, generic "craft community" prose). Replace each hit that
points users back at the upstream Craft Agents community with the
...
```
rg -in 'discord|twitter\.com|x\.com|community\.|forum\.|docs\..*craft' \
...
- **Generic-third-party Discord webhook documentation** (PRESERVE, not
  ...
  - `apps/electron/resources/docs/automations.md` — Discord prose and
    `CRAFT_WH_DISCORD_URL` example showing how arbitrary Discord
    webhooks integrate.
  - `packages/shared/src/automations/utils.ts:295` — `CRAFT_WH_DISCORD_TOKEN`
    JSDoc example.
- **Historical immutable artifacts** (PRESERVE):
  - `apps/electron/resources/release-notes/0.9.0.md:46` — historical
    `docs.craft.do/messaging/lark` link in a shipped release note.
  - Many `lukilabs/craft-agents-oss/issues/<n>` references throughout
...
- **Developer-facing traceability comment** (line-allowlisted):
  - `packages/server-core/src/sessions/SessionManager.ts:680` —
    `See: https://github.com/lukilabs/craft-agents-oss/issues/39` is a
...
regex (which uses `discord|twitter\.com|x\.com|community\.|forum\.|docs\..*craft`)
because the URL is a GitHub upstream-repo link without `discord/twitter/x/community/forum`
in the path. The audit test treats `https://github.com/lukilabs/craft-agents-oss`
...
| --- | --- | --- |
| `upstream-issues-link` | `https://github.com/lukilabs/craft-agents-oss/issues/\d+` | upstream issue refs outside historical records (line-allowlist for `SessionManager.ts:680`). |
| `upstream-repo-tab` | `https://github.com/lukilabs/craft-agents-oss(?!\.git\|/issues\|/pulls)\b` | standalone upstream-repo references outside attribution. |
| `upstream-pulls-tab` | `https://github.com/lukilabs/craft-agents-oss/pulls\b` | demo browser tabs / similar. |
...
| `craft-community-text` | `\b(?:Cr\|cr)aft (community\|forum\|discord)\b` (case-insensitive) | literal craft-community prose. |
...
- []
+ [
+   ".github/ISSUE_TEMPLATE/bug_report.yml:116 [upstream-repo-tab] Launch the app with `-- --debug` and reproduce the issue. See the [Troubleshooting section in the README](https://github.com/lukilabs/craft-agents-oss#troubleshooting) for platform-specific commands and log file locations.",
+   "apps/electron/src/renderer/playground/registry/browser-ui.tsx:691 [upstream-pulls-tab] url: 'https://github.com/lukilabs/craft-agents-oss/pulls',",
+   "apps/electron/src/renderer/playground/registry/browser-ui.tsx:783 [upstream-repo-tab] url: 'https://github.com/lukilabs/craft-agents-oss',",
+ ]
...
| --- | --- | --- |
| `.github/ISSUE_TEMPLATE/bug_report.yml:116` | `https://github.com/lukilabs/craft-agents-oss#troubleshooting` | `https://github.com/agisota/rox-one-terminal#troubleshooting` |
| `apps/electron/src/renderer/playground/registry/browser-ui.tsx:691` | `https://github.com/lukilabs/craft-agents-oss/pulls` | `https://github.com/agisota/rox-one-terminal/pulls` |
| `apps/electron/src/renderer/playground/registry/browser-ui.tsx:783` | `https://github.com/lukilabs/craft-agents-oss` | `https://github.com/agisota/rox-one-terminal` |
...
- **SessionManager.ts:680 line allowlist.** The narrow per-line
  ...
  exact substring `See: https://github.com/lukilabs/craft-agents-oss/issues/39`,
...
- **Discord/Slack docs in automations.md.** The `CRAFT_WH_DISCORD_URL`
  ...
  `CRAFT_WH_*` env-var namespace itself is owned by R.6 / future
