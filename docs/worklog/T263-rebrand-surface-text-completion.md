...
- Result: exit 1.
- Expected failures:
  - `Expected to not contain: "\"menu.roxMenu\""` in locale JSON.
  ...
  - Messaging playground demos still contained `Rox Agents`.

## 6. Implementation changes

- Renamed `menu.roxMenu` to `menu.appMenu` across all locale files and
  updated `TopBar.tsx`.
- Renamed `onboarding.apiSetup.roxAgentsBackend` to
...
- Updated bundled automations docs from `Rox Agent` / `~/.rox-agent/...`
  to `ROX.ONE` / `~/.rox/...`.
- Updated messaging playground demo labels from `Rox Agents` to `ROX.ONE`.
...
- `bun run electron:smoke`: exit 1 in this container after the build step
  ...
  `ROX_SERVER_URL=` and `App initialized successfully`.
...
| README has no active body `Rox Agents` product prose | Pass | R.1 regression test passed |
