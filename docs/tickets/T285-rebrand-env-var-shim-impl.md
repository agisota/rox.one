...
We are building a white-label fork of Craft Agents OSS into the ROX.ONE Agent
Workbench Suite. Phase R.6 of the rebrand sweep renames the 16 canonical
`CRAFT_*` environment variables to `ROX_*` and keeps the legacy names readable
for one minor version via a shared backward-compat shim.

Relevant goals:

- preserve runtime behavior for existing operators who already export `CRAFT_*`
...
2. Falls back to the matching `CRAFT_*` legacy name when the new `ROX_*` name
...
`CRAFT_` prefix (see `scripts/validate-rebrand.cjs:73`).
...
- [x] No new `CRAFT_*` findings introduced outside the allowlist.
