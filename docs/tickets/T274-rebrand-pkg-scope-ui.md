...
Rename `@craft-agent/ui` to `@rox-one/ui` across UI package metadata, app
...
it fails on the current `@craft-agent/ui` package name.
...
- Update active imports, subpath imports, style imports, Vite/Vitest config
  references, and package comments from `@craft-agent/ui` to `@rox-one/ui`.
