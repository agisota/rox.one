...
- Detects `~/.craft-agent/` first, then `~/.craft/` (per spec priority).
- Copies the legacy tree to `~/.rox/` only when the destination does
  not exist.
- Writes a `.migrated-from-craft` marker after a successful copy.
...
2. Legacy `~/.craft-agent/` exists, `~/.rox/` does not → copies tree,
...
5. Priority: when both `~/.craft-agent/` and `~/.craft/` exist (and
   `~/.rox/` does not), the source is `~/.craft-agent/`.
