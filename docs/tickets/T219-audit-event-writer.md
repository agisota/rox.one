...
- Shared audit storage exports live under `@craft-agent/shared/audit` so shared
  scope emitters do not depend on server-core.
- `@craft-agent/server-core/audit` continues to expose the same schema by
...
3. Prove persistent audit append does not require `CRAFT_DEBUG`.
...
- [x] Persistent append works without `CRAFT_DEBUG`.
