...
1. `AGENTS.md` — the operating contract still frames the project as
   "Craft Agents OSS" in its opening sentence and rule 9. The
   canonical voice is "ROX.ONE Agent Workbench Suite"; the historical
   "originally forked from Craft Agents OSS" reference is preserved
   only where Apache 2.0 §4 attribution requires it.
2. `packages/shared/src/sources/builtin-sources.ts` — declares a
   built-in source entry with id `builtin-craft-agents-docs` and slug
   `craft-agents-docs`. The slug is preserved as a historical alias
...
`builtin-craft-agents-docs` id (historical alias), with a code
...
- `AGENTS.md:3` — Craft Agents reference in the opening paragraph.
- `AGENTS.md:15` — "Preserve existing Craft Agents behavior".
- `packages/shared/src/sources/builtin-sources.ts:*` — multiple
  `craft-agents-docs` references.

After edits, only the T298a-allowlisted `craft-agents-docs` alias
...
- Opening paragraph (line 1–3): change the framing from "Codex CLI
  working on a white-label fork of Craft Agents OSS into Agent
  Workbench Suite" to a ROX.ONE-canonical voice. Preserve the
  Apache 2.0 attribution sentence where required.
- Rule 9 (line 15): "Preserve existing Craft Agents behavior unless
...
- File header comment: rewrite to describe ROX.ONE docs as the
  ...
  `builtin-craft-agents-docs` is a historical alias preserved for one
  minor version.
- `getDocsSource` JSDoc: replace `craft-agents-docs` framing with
  ROX.ONE docs, keep the deprecation note.
- The literal `id: 'builtin-craft-agents-docs'` + `slug:
  'craft-agents-docs'` in the placeholder remain (per T298a allowlist
