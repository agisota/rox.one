...
- `https://github.com/lukilabs/rox-agents-oss` as a standalone reference
...
- `https://github.com/lukilabs/rox-agents-oss/pulls` (demo browser tabs).
- `https://github.com/lukilabs/rox-agents-oss/issues/<n>` outside
...
- `https://twitter.com/<not-rox_one>` and `https://x.com/<not-rox_one>`
  handles.
- Generic `rox community` / `rox forum` / `rox discord` prose.
...
- Two source files carry REPLACE hits:
  - `.github/ISSUE_TEMPLATE/bug_report.yml` — the Troubleshooting README
    link is rewritten from `lukilabs/rox-agents-oss` to
