...
That means broad imports from `@craft-agent/shared/config` can evaluate the PI
...
- Add an explicit `@craft-agent/shared/config/models-pi` package export.
- Remove `models-pi` from the broad `@craft-agent/shared/config` barrel.
...
| `@craft-agent/shared/config` no longer exports `models-pi` | DONE |
| `@craft-agent/shared/config/models-pi` is exported explicitly | DONE |
