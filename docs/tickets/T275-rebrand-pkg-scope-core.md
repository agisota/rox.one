...
Rename `@craft-agent/core` to `@rox-one/core` across core package metadata,
...
it fails on the current `@craft-agent/core` package name.
...
- Update active imports, type imports, package docs/comments, and tsconfig
  path mappings from `@craft-agent/core` to `@rox-one/core`.
