...
surfaces that still expose legacy `craft-agent` / `craftagents` naming after
...
- stale package-copy references to removed `packages/craft-*` directories are
  removed
...
- [ ] Removed Dockerfile references to nonexistent `packages/craft-*` dirs.
