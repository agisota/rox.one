...
workspace packages have moved from `@rox-agent/*` to `@rox-one/*`.
The remaining app package names are `@rox-agent/cli`,
`@rox-agent/electron`, `@rox-agent/viewer`, and `@rox-agent/webui`.
...
- Do not rename generic bare `rox-agent` ESLint plugin identifiers, CLI
