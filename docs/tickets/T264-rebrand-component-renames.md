...
Phase R.2 removes product-legacy `Rox*` identifiers from active UI component
code before package-scope and asset-path phases begin.

## Goal

Rename the active icon component identifiers from `Rox*` to `Rox*` while
preserving their rendered behavior and consumer imports.

## Required UI

- `RoxAppIcon` -> `RoxAppIcon`.
- `RoxAgentsLogo` -> `RoxAgentsLogo`.
...
it fails on the existing `Rox*` component identifiers.
...
- [x] No active `RoxAppIcon`, `RoxAgentsLogo`, or `RoxAgentsSymbol`
