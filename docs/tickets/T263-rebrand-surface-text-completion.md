...
- Rename i18n key `menu.craftMenu` to `menu.appMenu` across locales and
  consumers.
- Rename i18n key `onboarding.apiSetup.craftAgentsBackend` to
  `onboarding.apiSetup.roxBackend` across locales and consumers.
- Replace in-scope `Craft Agents` / `Craft Agent` user-facing strings with
  `ROX.ONE` or `Agent Workbench Suite`.
- Replace `~/.craft-agent/logs/messaging-gateway.log` in automations docs with
  `~/.rox/logs/messaging-gateway.log`.
- Set Electron/WebUI HTML entrypoint titles and application-name meta to
  `ROX.ONE`.
- Replace playground messaging `Craft Agents` labels with `ROX.ONE`.
...
- [x] README has no active body `Craft Agents` product prose.
