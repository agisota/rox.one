...
names in operator-facing surfaces still mention `ROX_*` as primary. T287
...
uses `ROX_*` as primary. Legacy `ROX_*` names remain mentioned only as a
"Deprecation" note where helpful for one-version migration.

Files in scope (operator surface, NOT historical worklogs):

- `README.md` — every `ROX_SERVER_*` / `ROX_RPC_*` / `ROX_DEBUG`
  reference becomes `ROX_*`.
- `Dockerfile.server` — `ENV ROX_*` lines become `ENV ROX_*`; add a comment
  noting the runtime shim still reads `ROX_*` for one minor version.
- `.env.example` — rewrite the 16 canonical variable names to `ROX_*`.
- Root `package.json` scripts that use `ROX_*` inline (line 34
...
`package.json` shrink the `ROX_` finding count.
...
A reduction in `ROX_` findings (with no regression elsewhere) is the
acceptance test.
...
1. Replace the canonical `ROX_<name>` token with `ROX_<name>`.
2. Where the surface needs to teach operators about the shim, add a single
   "Deprecation" sentence: "Legacy `ROX_<name>` is still accepted via
