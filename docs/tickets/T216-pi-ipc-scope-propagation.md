...
the same tenant storage root, and rejects crafted cross-workspace envelopes in
multi-tenant mode.
...
2. Add a Pi-side forgery test that proves a crafted workspace id outside
...
- [x] Crafted cross-workspace envelopes reject with `MultiTenantForgeryError`.
