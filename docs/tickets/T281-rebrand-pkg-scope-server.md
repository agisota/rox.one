...
`@rox-agent/server` is package metadata and lockfile only, while
`@rox-agent/server-core` is used by package metadata, workspace dependencies,
tsconfig paths, source imports, tests, and the server build alias.

## Goal

Rename `@rox-agent/server` and `@rox-agent/server-core` to
...
it fails on the current `@rox-agent/server` and `@rox-agent/server-core`
names.
...
- Rename `packages/server-core/package.json` `name` to
  `@rox-one/server-core`.
- Rename active `@rox-agent/server-core` workspace dependencies to
  `@rox-one/server-core`.
- Rename active `@rox-agent/server-core` imports and dynamic imports to
  `@rox-one/server-core`.
...
- Do not rename `@rox-agent/shared` or app package scopes in this ticket.
