...
Legacy `CRAFT_*` env-var keys in workflow YAML (`CRAFT_E2E_FAKE_PROVIDERS`,
`CRAFT_HEADLESS`, `CRAFT_DEV_RUNTIME`, the `CRAFT_ANTHROPIC_API_KEY`
...
edit reintroduces `Craft` in a name key or `craft-agent-` as an artifact
prefix.
...
1. No file under `.github/workflows/` has a `name:` key whose value
   contains `Craft` or `craft-agent` (case-insensitive `Craft` plus
   exact `craft-agent`).
2. No file under `.github/workflows/` contains any `craft-agent-*`
   artifact prefix.
...
Do NOT rename CRAFT_* env-var keys in workflow YAML during R.7; those
are intentionally preserved to keep the R.6 `readEnv()` shim's
deprecation-warning code path exercised in CI.

External GitHub secret names (e.g. `CRAFT_ANTHROPIC_API_KEY`) are owned
...
- [ ] CRAFT_* env-var keys remain in workflow YAML to exercise the
      R.6 shim.
