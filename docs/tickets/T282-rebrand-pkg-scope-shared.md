...
server-core package scopes have landed. `@rox-agent/shared` remains the active
...
Rename `@rox-agent/shared` to `@rox-one/shared` across active package
...
it fails on the current `@rox-agent/shared` package name.

## Implementation Requirements

- Rename `packages/shared/package.json` `name` to `@rox-one/shared`.
- Rename active `@rox-agent/shared` workspace dependencies to
  `@rox-one/shared`.
- Rename active `@rox-agent/shared` imports and dynamic imports to
  `@rox-one/shared`.
