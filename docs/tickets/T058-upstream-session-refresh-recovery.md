...
Upstream Rox Agents OSS v0.9.0 includes a safe reconnect recovery fix for session metadata refresh. During stale reconnects after sleep/wake, `getSessions()` can transiently return a partial list. Treating that partial response as authoritative collapses the sidebar to only returned sessions.
