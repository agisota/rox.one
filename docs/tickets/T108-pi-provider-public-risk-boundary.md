...
- Derive `public-untrusted` from `ROX_PUBLIC_APP_URL` when no explicit PI
  provider risk mode is configured.
- Allow explicit override through `ROX_PI_PROVIDER_DEPENDENCY_RISK_MODE` or
  generic `ROX_PROVIDER_DEPENDENCY_RISK_MODE`.
