...
3. Upstream-owned: accept upstream, then translate any `CRAFT_*` /
   `craft-agent` / `CraftAgent*` survival via the audit §8 pattern card.
...
- **i18n parity:** `bun run lint:i18n:parity`; fill missing keys per
  audit §4.2.
- **Env-var shim:** for each new `CRAFT_*`, append one-line mapping to
  `packages/shared/src/config/env-shim.ts` and one row to
  `docs/release/rebrand-mapping-2026-05-13.md`.
- **Package scope:** `rg -F '@craft-agent/' --type ts --type json` should
...
- **Brand text:** every `Craft Agents` literal → `ROX.ONE`.
