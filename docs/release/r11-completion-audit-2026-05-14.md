...
| Default R.11 preflight | `bun run rebrand:r11-preflight` | Exits red on active goal acknowledgement, PR #214, fork review, tag blockers, and non-main checkout | Blocked |
| Pre-rewrite R.11 preflight | `ROX_R11_NO_ACTIVE_GOAL=1 bun run rebrand:r11-preflight --stage pre-rewrite` | Exits red on PR #214, fork review, tag blockers, missing backup artifacts, remote branch review, and non-main checkout | Blocked |
...
| 11. main sync | `bun run rebrand:r11-preflight` reports `main-sync` pass with local `main` and `origin/main` both at `2fa129f3` and `origin/main...main` at `0 0`. | Green |
...
- `bun run rebrand:r11-preflight` exits red with 6 blockers:
  ...
  `rebrand-tag-local-sync`, `rebrand-tag-on-main`, and `current-branch`; open
  ...
  `current-branch` fails because Current checkout is report/r11-t463-consolidation-backlog;
  `main-sync` passes because
  origin/main...main is 0 0; `worktree-clean` passes with `git status
  --porcelain is empty`.
- `ROX_R11_NO_ACTIVE_GOAL=1 bun run rebrand:r11-preflight --stage pre-rewrite`
  exits red with 9 blockers: `no-open-prs`, `fork-review`,
  `rebrand-tag-local-sync`, `rebrand-tag-on-main`, `backup-tag`,
  `backup-branch`, `offline-mirror`, `remote-branch-review`,
  and `current-branch`; GitHub reports 1 fork(s); expected 0; the
  ...
  any `git filter-repo` invocation. The `worktree-clean` row passes here too:
