...
merge of `v0.9.3` from `craft-ai-agents/craft-agents-oss` into the
...
| Field | Value |
| --- | --- |
| Upstream repo | `https://github.com/craft-ai-agents/craft-agents-oss.git` |
...
| Local legal-preserve attribution URL | `https://github.com/lukilabs/craft-agents-oss` (README §License; immutable until R.11) |
...
repo renamed `rox-agents-oss` → `craft-agents-oss` between v0.9.1 and v0.9.3.
The legal-preserve attribution URL (`lukilabs/craft-agents-oss` in README,
...
Every `CRAFT_*` env-var, `@craft-agent/*` import, `CraftAgent*` identifier,
`craft-cli` binary name, and `~/.craft/` path that upstream re-introduces in
...
`CRAFT_RBAC_*` token to its `Rox*` / `rox-rbac` / `ROX_RBAC_*` equivalent
...
| `packages/shared/src/sources/credential-manager.ts` | 1 | Preserve `RoxOAuth` after R.2 rename; reject upstream `CraftOAuth` rename. |
...
| Surface | Files touched (estimate) | ROX canonical pattern |
| --- | ---: | --- |
| `apps/electron/src/renderer/components/icons/RoxAppIcon.tsx` (and `RoxAgentsLogo`, `RoxAgentsSymbol`) | 3 | Preserve ROX-renamed component names; reject upstream `CraftAgentsLogo` revival. |
| `apps/electron/index.html` (title, meta) | 1 | `<title>ROX.ONE</title>` + `application-name=ROX.ONE` (R.1 baseline). |
| `apps/electron/src/renderer/pages/settings/AiSettingsPage.tsx`, `MessagingSettingsPage.tsx` | 2 | Merge upstream behavioural changes; reject any settings strings carrying `Craft Agents` literals. |
| Playground demos (`apps/electron/src/renderer/playground/demos/messaging/*.tsx`) | 2–3 | Preserve `"ROX.ONE"` literals; reject upstream `"Craft Agents"` revival. |
...
| Every workspace `package.json` (`packages/*/package.json`, `apps/*/package.json`) | 12 | Preserve `@rox-one/*` scope; reject any upstream `@craft-agent/*` revival. |
...
| User-data path `~/.rox/` (R.8 migration shim) | source: `packages/shared/src/config/paths.ts` | Reject any upstream `~/.craft/` patch that bypasses the migration shim. |

**Re-apply pattern (env-vars):** for every `CRAFT_*` env-var upstream
introduces, write a one-line `readEnv()` mapping in
`packages/shared/src/config/env-shim.ts` (the R.6 surface) that translates
`CRAFT_FOO` → `ROX_FOO` with the deprecation warning. Document the new
...
| --- | --- | --- |
| `CRAFT_*` (env-var prefix) | `ROX_*` (route through `readEnv()` shim) | ADR 0011 + R.6 |
| `craft-agent` (package scope segment) | `rox-one` | ADR 0011 + R.5 |
| `@craft-agent/*` (npm scope) | `@rox-one/*` | ADR 0011 + R.5 |
| `CraftAgent*` (TS identifiers) | `RoxAgent*` (backward-compat alias kept where re-exported) | ADR 0011 + R.2 |
| `CraftAppIcon`, `CraftAgentsLogo`, `CraftAgentsSymbol` | `RoxAppIcon`, `RoxAgentsLogo`, `RoxAgentsSymbol` | R.2 |
| `CraftMcpClient`, `CraftOAuth`, `CraftMetadataSchema` | `RoxMcpClient`, `RoxOAuth`, `RoxMetadataSchema` | R.2 |
| `craft-cli` (binary name) | `rox-cli` | R.3 |
| `craft-logos/` (asset dir) | `rox-logos/` | R.3 |
| `~/.craft-agent/`, `~/.craft/` (user-data path) | `~/.rox/` (migration shim handles legacy `~/.rox-agent/`) | R.8 |
| `Craft Agents`, `Craft Agent` (product text) | `ROX.ONE` (wordmark) or `Agent Workbench Suite` (suite descriptor) | ADR 0011 + R.1 |
