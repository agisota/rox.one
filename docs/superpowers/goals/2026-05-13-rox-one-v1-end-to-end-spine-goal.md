...
| R | R.3 | Asset file renames (`rox-logos/`, binaries, doc filenames) | NEXT | rebrand-sweep | T267-T268 | — |
| R | R.4 | Documentation / decision-record / plan cleanup | QUEUED | rebrand-sweep | T269-T272 | — |
| R | R.5 | Package scope rename `@rox-agent/*` → `@rox-one/*` (11 sub-phases) | QUEUED | rebrand-sweep | T273-T284 | — |
| R | R.6 | Env-var rename `ROX_*` → `ROX_*` with `readEnv()` shim | QUEUED | rebrand-sweep | T285-T288 | — |
| R | R.7 | Docker / CI / build rebrand | QUEUED | rebrand-sweep | T289-T291 | — |
| R | R.8 | User-data migration shim `~/.rox/` → `~/.rox/` | QUEUED | rebrand-sweep | T292-T294 | — |
...
Weekly GitHub Action that fetches `upstream/main` from `rox-ai-agents/rox-agents-oss`, runs `git merge --no-ff --strategy=ort -X theirs upstream/main` into a `chore/upstream-weekly-<date>` branch, runs the global validation matrix, and opens a PR if green. If conflicts arise in any protected ROX-owned surface (per the list in the rebrand-sweep R.4 phase and `plan.md §6.2`), the job stops and pings the owners.
