...
8. The origin `rebrand-v1` target is on `origin/main` ancestry.
9. The local `rebrand-v1` tag target matches origin's `rebrand-v1` target.
10. Local working tree is clean: `git status --porcelain` → empty.
11. Local `main` is in sync with `origin/main`: `git rev-list --left-right --count origin/main...HEAD` → `0	0`.
...
The backup tag, backup branch, and offline mirror must **all** exist before any filter-repo invocation. The backup tag and backup branch targets must match `main`; the offline mirror `main` target must match `main`. The explicit pre-rewrite helper reports these as `backup-tag-target`, `backup-branch-target`, and `offline-mirror-target`. Confirm with `ls -la /tmp/rox-one-terminal-backup-2026-05-13.git`, `git ls-remote --tags origin | grep pre-rebrand`, and `git ls-remote --heads origin backup/pre-rebrand-history-rewrite-2026-05-13`, then run the explicit pre-rewrite helper gate:
...
- Backup tag, backup branch, and backup mirror all exist; the backup tag and
  backup branch targets match `main`, and the offline mirror `main` target
  matches `main`.
...
6. `pre-rebrand-history-rewrite-backup` tag and `backup/pre-rebrand-history-rewrite-2026-05-13` branch exist on `origin`, both targets match `main`, and the offline mirror at `/tmp/rox-one-terminal-backup-2026-05-13.git` has a matching `main` target and is preserved for at least 90 days.
