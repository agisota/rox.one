...
state under `~/.rox-agent/` (older `rox-agent` installs) or `~/.rox/`
...
### 2.1 Goals
- Single-user upgrade path: a user with `~/.rox-agent/` and no `~/.rox/`
...
1. `~/.rox-agent/` (older — was the brand at v0.5.x)
2. `~/.rox/` (intermediate — was the brand at v0.7.x–v0.8.x)
...
The marker file is `<newRoot>/.migrated-from-rox`. Plain ASCII, two
lines:

```
migrated-from: /home/<user>/.rox-agent
...
Additional case: the priority order — when both `~/.rox-agent/` and
`~/.rox/` exist (and `~/.rox/` does not), `~/.rox-agent/` is the
source.
...
- **Marker tampering**: a user who deletes `~/.rox/.migrated-from-rox`
