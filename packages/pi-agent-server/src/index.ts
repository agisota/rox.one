...
function setInterceptorApiHints(model: { api?: string; provider?: string; baseUrl?: string } | undefined): void {
  if (!model) {
    delete process.env.ROX_PI_MODEL_API;
    delete process.env.ROX_PI_MODEL_PROVIDER;
    delete process.env.ROX_PI_MODEL_BASE_URL;
    return;
  }

  process.env.ROX_PI_MODEL_API = model.api || '';
  process.env.ROX_PI_MODEL_PROVIDER = model.provider || '';
  process.env.ROX_PI_MODEL_BASE_URL = model.baseUrl || '';

  debugLog(
    `[interceptor-hint] api=${process.env.ROX_PI_MODEL_API || '-'} provider=${process.env.ROX_PI_MODEL_PROVIDER || '-'} baseUrl=${process.env.ROX_PI_MODEL_BASE_URL || '-'}`,
  );
}
...
async function ensureSession(): Promise<AgentSession> {
  ...
  if (initConfig.sessionPath) {
    ...
    // Session resume: use a per-Rox-session directory so the Pi SDK can
...
function wrapSingleTool(tool: ToolDefinition<any, any>): ToolDefinition<any, any> {
  ...
  const parameters = allowRoxAgentMetadataProperties(tool.parameters);
  ...
  const wrappedExecute: ToolDefinition<any, any>['execute'] = async (
    ...
    // Metadata is for Rox UI only. Keep a final defensive strip here so the
    ...
    inputObj = stripRoxAgentMetadata(inputObj);
 ...
 * (see rox-agents-oss#464). Default timeout matches the RPC compact timeout
...
async function handlePrompt(msg: Extract<InboundMessage, { type: 'prompt' }>): Promise<void> {
  ...
  try {
    ...
    // Wait for any in-flight auto-compaction to avoid race (rox-agents-oss#464)
    ...
    // plans/fix-pi-gpt-compaction.md). PiEventAdapter holds the Rox event
