...
describe('bootstrapScope', () => {
  ...
  const previousToken = process.env.CRAFT_PI_SCOPE_IPC_TOKEN
  ...
  beforeEach(() => {
    ...
    process.env.CRAFT_PI_SCOPE_IPC_TOKEN = 'token-1'
  })
  ...
  afterEach(() => {
    ...
    if (previousToken === undefined) delete process.env.CRAFT_PI_SCOPE_IPC_TOKEN
    else process.env.CRAFT_PI_SCOPE_IPC_TOKEN = previousToken
    if (tempConfigDir) {
  ...
  it('rejects a crafted requested workspace outside the permitted set', () => {
    expect(() => {
      bootstrapScope({
