const ROX_DISPLAY_NAME_KEY = '_displayName';
const ROX_INTENT_KEY = '_intent';

const ROX_DISPLAY_NAME_SCHEMA = {
  type: 'string',
  description: 'Rox UI metadata: human-friendly action name for display only.',
};

const ROX_INTENT_SCHEMA = {
  type: 'string',
  description: 'Rox UI metadata: concise tool-call intent for display only.',
};
 ...
 */
export function allowRoxMetadataProperties<T>(schema: T): T {
  ...
  if (!(ROX_DISPLAY_NAME_KEY in nextProperties)) {
    nextProperties[ROX_DISPLAY_NAME_KEY] = ROX_DISPLAY_NAME_SCHEMA;
  }
  if (!(ROX_INTENT_KEY in nextProperties)) {
    nextProperties[ROX_INTENT_KEY] = ROX_INTENT_SCHEMA;
  }
...
export function stripRoxMetadata<T>(input: T): T {
  if (!isRecord(input)) return input;
  if (!(ROX_DISPLAY_NAME_KEY in input) && !(ROX_INTENT_KEY in input)) return input;

  const cleanInput = { ...input };
  delete cleanInput[ROX_DISPLAY_NAME_KEY];
  delete cleanInput[ROX_INTENT_KEY];
