...
import { registerRoxShikiThemes } from './registerShikiThemes'
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to rox-dark/rox-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to rox-dark/rox-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'rox-dark' : 'rox-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__roxClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__roxClickCleanup?.()
      }
    }
