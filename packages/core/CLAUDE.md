...
This file provides guidance to Claude Code when working with the `@rox-agent/core` package.
...
The core package provides shared TypeScript types and utilities used by the Electron app and shared packages. It serves as the type definition layer for the Rox Agent monorepo.
...
| `AgentEvent` | Events from RoxAgent during chat (incl. task_backgrounded, shell_backgrounded, task_progress) |
...
import type { Session } from '@rox-agent/core/types';
```
...
**Critical:** Rox OAuth (`rox_oauth::global`) is ONLY for the Rox API (managing spaces, MCP links). It is NEVER used for MCP server authentication. Each MCP server has its own OAuth via `workspace_oauth::{workspaceId}`.
...
2. **Phase 2:** Move storage logic to `@rox-agent/core`
