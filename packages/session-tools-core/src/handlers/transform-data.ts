...
export async function handleTransformData(
  ...
  const tempScript = join(tmpdir(), `rox-transform-${ctx.sessionId}-${Date.now()}${ext}`);
  writeFileSync(tempScript, args.script, 'utf-8');

  try {
    // Build command
    // Use bundled uv for Python when available (ensures consistent Python 3.12 + deps)
    const useUv = args.language === 'python3' && !!process.env.ROX_UV;
    const cmd = useUv ? process.env.ROX_UV! : (args.language === 'python3' ? 'python3' : args.language);
