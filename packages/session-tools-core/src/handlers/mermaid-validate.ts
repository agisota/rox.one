 ...
 *
 * Validates Mermaid diagram syntax using @rox-agent/mermaid parser.
 * No DOM required - works identically in Claude and Codex.
 */

import type { SessionToolContext } from '../context.ts';
import type { ToolResult } from '../types.ts';
import { parseMermaid } from '@rox-agent/mermaid';
 ...
 *
 * Uses parseMermaid from @rox-agent/mermaid to validate syntax.
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          valid: false,
          error: errorMessage,
          suggestion: 'Check the syntax against ~/.rox-agent/docs/mermaid.md',
        }, null, 2),
      }],
      isError: true,
    };
