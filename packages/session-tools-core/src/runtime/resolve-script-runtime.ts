...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to ROX_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.ROX_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.ROX_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.ROX_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute ROX_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute ROX_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (ROX_UV / ROX_NODE / ROX_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.ROX_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ROX_UV, 'Python/uv')
        : process.env.ROX_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure ROX_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.ROX_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.ROX_NODE, 'Node')
        : process.env.ROX_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure ROX_NODE or install node on PATH.'
    );
  }

  if (process.env.ROX_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.ROX_BUN, 'Bun')
      : process.env.ROX_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure ROX_BUN or install bun on PATH.'
  );
}
