...
describe('McpClientPool.sync — config change detection', () => {
  ...
  it('reconnects when Authorization header changes (token refresh)', async () => {
    await pool.sync({ rox: httpConfig('old-token') });
    expect(pool.isConnected('rox')).toBe(true);
    pool.resetTracking();

    await pool.sync({ rox: httpConfig('new-token') });

    expect(pool.disconnectCalls).toEqual(['rox']);
  ...
  it('reconnects when URL changes', async () => {
    await pool.sync({ rox: httpConfig('token', 'https://old.example.com') });
    pool.resetTracking();

    await pool.sync({ rox: httpConfig('token', 'https://new.example.com') });
  ...
  it('does not reconnect when only non-auth headers change', async () => {
    ...
    await pool.sync({ rox: config1 });
    pool.resetTracking();

    await pool.sync({ rox: config2 });
  ...
  it('disconnects sources removed from config', async () => {
    ...
    await pool.sync({ rox: config, linear: config });
    pool.resetTracking();
  ...
  it('handles add + remove + refresh in a single sync', async () => {
    await pool.sync({
      rox: httpConfig('old-rox-token'),
      linear: httpConfig('linear-token', 'https://linear.example.com'),
    });
    pool.resetTracking();

    // rox: token refreshed, linear: removed, github: added
    await pool.sync({
      rox: httpConfig('new-rox-token'),
      github: httpConfig('gh-token', 'https://github.example.com'),
    });

    expect(pool.disconnectCalls).toContain('linear');
    expect(pool.disconnectCalls).toContain('rox');
    expect(pool.connectCalls.find(c => c.slug === 'rox')?.config.headers?.Authorization).toBe('Bearer new-rox-token');
  ...
  it('reports failure when reconnect fails after token refresh', async () => {
    ...
    await failPool.sync({ rox: httpConfig('old-token') });
    expect(failPool.isConnected('rox')).toBe(true);

    // Token refresh — disconnect succeeds but reconnect throws
    const failures = await failPool.sync({ rox: httpConfig('new-token') });

    expect(failures).toContain('rox');
    expect(failPool.isConnected('rox')).toBe(false);
  });
});
