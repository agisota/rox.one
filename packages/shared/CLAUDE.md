...
`@rox-agent/shared` is the core business logic package for Rox Agent. It contains:
- Agent implementation (RoxAgent, session-scoped tools, permission modes)
...
```typescript
import { RoxAgent, getPermissionMode, setPermissionMode } from '@rox-agent/shared/agent';
import { loadStoredConfig, type Workspace } from '@rox-agent/shared/config';
import { getCredentialManager } from '@rox-agent/shared/credentials';
import { RoxMcpClient } from '@rox-agent/shared/mcp';
import { loadWorkspaceSources, type LoadedSource } from '@rox-agent/shared/sources';
import { loadStatusConfig, createStatus } from '@rox-agent/shared/statuses';
import { resolveTheme } from '@rox-agent/shared/config/theme';
...
```
src/
├── agent/              # RoxAgent, session-scoped-tools, mode-manager, mode-types, permissions-config
├── auth/               # OAuth, rox-token, claude-token, state
...
├── subscription/       # Rox subscription checking
...
### RoxAgent (`src/agent/rox-agent.ts`)
...
- Workspace: `~/.rox-agent/workspaces/{id}/permissions.json`
- Source: `~/.rox-agent/workspaces/{id}/sources/{slug}/permissions.json`
...
**Storage:** `~/.rox-agent/workspaces/{id}/statuses/config.json`
...
**Storage:**
- App: `~/.rox-agent/theme.json`
- Workspace: `~/.rox-agent/workspaces/{id}/theme.json`
...
All sensitive credentials (API keys, OAuth tokens) are stored in an AES-256-GCM encrypted file at `~/.rox-agent/credentials.enc`. The `CredentialManager` provides the API for reading and writing credentials.
...
│     └── ~/.rox-agent/workspaces/{ws}/sources/{slug}/          │
└─────────────────────────────────────────────────────────────────┘
                              │
...
Multi-workspace configuration stored in `~/.rox-agent/config.json`. Supports:
...
### Sources (`src/sources/`)
Sources are external data connections (MCP servers, APIs, local filesystems). Stored at `~/.rox-agent/workspaces/{id}/sources/{slug}/` with config.json and guide.md. Types: `mcp`, `api`, `local`, `gmail`.

## Dependencies

- `@rox-agent/core` - Shared types
