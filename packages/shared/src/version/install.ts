...
export async function installArchive(params: { archiveData: ArrayBuffer, version: string }): Promise<void> {
  ...
  const versionDirectory = join(homedir(), '.local', 'share', 'rox', 'versions', version);
  const binaryPath = join(versionDirectory, 'rox');
  const symlinkDirectory = join(homedir(), '.local', 'bin');
  const symlinkPath = join(symlinkDirectory, 'rox');
