 ...
 *
 * Single-shot copy from legacy `~/.craft-agent/` (preferred) or
 * `~/.craft/` to the canonical `~/.rox/` directory on app startup.
 *
 * The shim is **non-destructive**: it always copies, never moves.
 * It writes a `.migrated-from-craft` marker inside the new root after
...
const DEFAULT_MARKER = '.migrated-from-craft'

function defaultLegacyRoots(): string[] {
  const home = homedir()
  return [join(home, '.craft-agent'), join(home, '.craft')]
}
