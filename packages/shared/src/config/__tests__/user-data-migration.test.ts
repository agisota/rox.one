 ...
 *
 * Plus the priority-order assertion: `~/.craft-agent/` wins over
 * `~/.craft/` when both legacy roots are present.
 *
 * Every test uses fixture filesystems via `mkdtempSync(tmpdir())` and
 * injects `legacyRoots` + `newRoot`. The real `~/.craft-agent/` /
 * `~/.craft/` / `~/.rox/` are never touched.
 */
...
describe('migrateUserDataIfNeeded', () => {
  it('Case 1: returns no-legacy-path when no legacy directory exists', () => {
    ...
    const legacyA = join(sandbox, '.craft-agent')
    const legacyB = join(sandbox, '.craft')
  ...
  it('Case 2: copies legacy ~/.craft-agent/ tree to ~/.rox/ and writes marker', () => {
    ...
    try {
      ...
      const markerPath = join(newRoot, '.migrated-from-craft')
  ...
  it('Case 3: warns and does nothing when both legacy and new root exist (no marker)', () => {
    ...
    try {
      ...
      // No marker written.
      expect(existsSync(join(newRoot, '.migrated-from-craft'))).toBe(false)
  ...
  it('Case 4: re-run after marker exists is an idempotent no-op', () => {
    ...
    mkdirSync(newRoot, { recursive: true })
    writeFileSync(
      join(newRoot, '.migrated-from-craft'),
    ...
    try {
      ...
      expect(
        readFileSync(join(newRoot, '.migrated-from-craft'), 'utf8'),
  ...
  it('Priority: ~/.craft-agent/ is preferred over ~/.craft/ when both exist', () => {
    ...
    writeFileSync(join(legacyA, 'config.json'), '{"from":"craft-agent"}\n', 'utf8')
    mkdirSync(legacyB, { recursive: true })
    writeFileSync(join(legacyB, 'config.json'), '{"from":"craft"}\n', 'utf8')
