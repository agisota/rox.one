...
export function buildClaudeSubprocessEnv(
    ...
    // Propagate debug mode from argv flag OR existing env var. Subprocesses
    // still on the legacy debug path are kept happy for one minor version by
    // also setting the computed legacy key below.
...
export function getDefaultOptions(envOverrides?: Record<string, string>): Partial<Options> {
    ...
    // SDK out at ~/.local/share/rox/versions/<version>/claude-agent-sdk/<binary>
    if (typeof ROX_AGENT_CLI_VERSION !== 'undefined' && ROX_AGENT_CLI_VERSION != null) {
        const baseDir = join(homedir(), '.local', 'share', 'rox', 'versions', ROX_AGENT_CLI_VERSION);
        return {
