...
export class CopilotAgent extends BaseAgent {
  ...
  private async ensureClient(): Promise<CopilotClient> {
    ...
    if (process.env.ROX_SESSION_DIR) {
      clientEnv.ROX_SESSION_DIR = process.env.ROX_SESSION_DIR;
    }

    // Propagate debug mode
    clientEnv.ROX_DEBUG = (process.argv.includes('--debug') || process.env.ROX_DEBUG === '1') ? '1' : '0';
    ...
    if (this.config.sessionServerPath) {
      ...
      if (sessionId && workspaceRootPath) {
        ...
        config['session'] = {
          ...
          env: this._callbackPort ? { ROX_LLM_CALLBACK_PORT: String(this._callbackPort) } : undefined,
          tools: ['*'],
        };
      }
    }
