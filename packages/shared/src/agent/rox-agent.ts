...
export class RoxAgent {
  ...
  async *chat(
    ...
    try {
      ...
      if (isMiniAgent) {
        debug('[RoxAgent] 🤖 MINI AGENT mode - optimized for quick config edits');
        debug('[RoxAgent] Mini agent optimizations:', {
          model,
          tools: ['Read', 'Edit', 'Write', 'Glob', 'Grep', 'Bash'],
          mcpServers: ['session', 'rox-agents-docs'],
          thinking: 'disabled',
      ...
      const options: Options = {
        ...
        tools: (() => {
          ...
          debug('[RoxAgent] 🔧 Tools configuration:', JSON.stringify(toolsValue));
          return toolsValue;
        })(),
        ...
        hooks: {
          ...
          SubagentStart: [{
            hooks: [async (input, _hookToolUseID) => {
              ...
              debug(`[RoxAgent] SubagentStart: agent_id=${typedInput.agent_id}, type=${typedInput.agent_type}`);
              return { continue: true };
            }],
          }],
          SubagentStop: [{
            hooks: [async (input, _toolUseID) => {
              ...
              debug(`[RoxAgent] SubagentStop: agent_id=${typedInput.agent_id}`);
              return { continue: true };
            }],
          }],
        },
