...
// Plan types are used by UI components; not needed in rox-agent.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.rox-agent/docs/
...
export interface RoxAgentConfig {
  workspace: Workspace;
...
export class RoxAgent {
  private config: RoxAgentConfig;
  ...
  // Temporary clarifications (not yet saved to Rox document)
  ...
  constructor(config: RoxAgentConfig) {
    ...
    debug(`[RoxAgent] Using model: ${model}`);
    ...
    registerSessionScopedToolCallbacks(sessionId, {
      onPlanSubmitted: (planPath) => {
        this.onDebug?.(`[RoxAgent] onPlanSubmitted received: ${planPath}`);
        this.onPlanSubmitted?.(planPath);
      },
      onAuthRequest: (request) => {
        this.onDebug?.(`[RoxAgent] onAuthRequest received: ${request.sourceSlug} (type: ${request.type})`);
  ...
  private startConfigWatcher(): void {
    ...
    this.configWatcher = createConfigWatcher(this.workspaceRootPath, {
      onSourceChange: (slug, source) => {
        debug('[RoxAgent] Source changed:', slug, source ? 'updated' : 'deleted');
        this.onSourceChange?.(slug, source);
      },
      onSourcesListChange: (sources) => {
        debug('[RoxAgent] Sources list changed:', sources.length);
        this.onSourcesListChange?.(sources);
      },
      onValidationError: (file, result) => {
        debug('[RoxAgent] Config validation error:', file, result.errors);
        this.onConfigValidationError?.(file, result.errors);
      },
      onError: (file, error) => {
        debug('[RoxAgent] Config file error:', file, error.message);
      },
    });

    debug('[RoxAgent] Config watcher started');
  }
  ...
  private stopConfigWatcher(): void {
    if (this.configWatcher) {
      ...
      debug('[RoxAgent] Config watcher stopped');
    }
  }
  ...
  private handleSourceUpdate(slug: string, source: LoadedSource | null): void {
    if (!source) {
      ...
      debug('[RoxAgent] Removed source:', slug);
      return;
    }
  ...
  setThinkingLevel(level: ThinkingLevel): void {
    ...
    this.onDebug?.(`[RoxAgent] Thinking level: ${level}`);
  }
  ...
  setUltrathinkOverride(enabled: boolean): void {
    ...
    this.onDebug?.(`[RoxAgent] Ultrathink override: ${enabled ? 'ENABLED' : 'disabled'}`);
  }
  ...
  async *chat(
    ...
    try {
      ...
      const mcpServers: Options['mcpServers'] = {
        ...
        // Rox Agents documentation - always available for searching setup guides
        ...
        // Note: Rox MCP server is now added via sources system
        ...
        hooks: {
            ...
            hooks: [async (input) => {
              ...
              if (input.tool_name.startsWith('mcp__')) {
                ...
                if (parts.length >= 3 && serverName) {
                  ...
                  // - rox-agents-docs: always-available documentation search
                  const builtInMcpServers = new Set(['preferences', 'session', 'rox-agents-docs']);
          ...
          SubagentStart: [{
            hooks: [async (input, _hookToolUseID) => {
              ...
              console.log(`[RoxAgent] SubagentStart: agent_id=${typedInput.agent_id}, type=${typedInput.agent_type}`);
              return { continue: true };
            }],
          }],
          SubagentStop: [{
            hooks: [async (input, _toolUseID) => {
              ...
              console.log(`[RoxAgent] SubagentStop: agent_id=${typedInput.agent_id}`);
              return { continue: true };
            }],
          }],
        },
      ...
      if (wasResuming) {
        console.error(`[RoxAgent] Attempting to resume SDK session: ${this.sessionId}`);
        debug(`[RoxAgent] Attempting to resume SDK session: ${this.sessionId}`);
      } else {
        console.error(`[RoxAgent] Starting fresh SDK session (no resume)`);
        debug(`[RoxAgent] Starting fresh SDK session (no resume)`);
      }
      ...
      try {
        ...
        console.error(`[RoxAgent] INNER CATCH triggered: ${sdkError instanceof Error ? sdkError.message : String(sdkError)}`);
        ...
        if (isConfigCorruption && !_isRetry) {
          debug('[RoxAgent] Detected .claude.json corruption, repairing and retrying...');
        ...
        if (isProcessError) {
          ...
          if (isSessionExpired && wasResuming && !_isRetry) {
            ...
            console.error('[RoxAgent] SDK session expired server-side, clearing and retrying fresh');
            debug('[RoxAgent] SDK session expired server-side, clearing and retrying fresh');
            this.sessionId = null;
      ...
      console.error(`[RoxAgent] OUTER CATCH triggered: ${error instanceof Error ? error.message : String(error)}`);
      console.error(`[RoxAgent] Error stack: ${error instanceof Error ? error.stack : 'no stack'}`);
...
`;
  ...
  private convertSDKMessage(
    ...
    switch (message.type) {
          ...
          if (block.type === 'text') {
            ...
            if (!emittedToolStarts.has(block.id)) {
              ...
              if (isParentTool) {
                ...
                console.log(`[RoxAgent] CHILD REGISTERED (assistant/single-parent): ${block.name} (${block.id}) under parent ${parentId}`);
              } else if (activeParentTools.size > 1) {
                ...
                if (sdkParentId && activeParentTools.has(sdkParentId)) {
                  ...
                  console.log(`[RoxAgent] CHILD REGISTERED (assistant/sdk-parent): ${block.name} (${block.id}) → Task (${sdkParentId})`);
                } else {
                  ...
                  childToParent.set(block.id, parentId);
                  parentToolUseId = parentId;
                  console.log(`[RoxAgent] CHILD REGISTERED (assistant/fifo-fallback): ${block.name} (${block.id}) → ${parentId} (sdk_parent=${sdkParentId})`);
                }
              }
      ...
      case 'stream_event': {
        ...
        if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') {
          ...
          if (!emittedToolStarts.has(toolBlock.id)) {
            ...
            console.log(`[RoxAgent] TOOL START: ${toolBlock.name} (${toolBlock.id}), sdk_parent=${sdkParentId}, activeParents=[${Array.from(activeParentTools).join(',')}]`);
            ...
            if (isParentTool) {
              ...
              console.log(`[RoxAgent] PARENT REGISTERED (stream): ${toolBlock.name} (${toolBlock.id})`);
              ...
              console.log(`[RoxAgent] CHILD REGISTERED (stream/sdk): ${toolBlock.name} (${toolBlock.id}) under parent ${sdkParentId}`);
            } else if (activeParentTools.size === 1) {
              // Single active parent - unambiguous, assign directly
              const parentId = Array.from(activeParentTools)[0]!;
              ...
              parentToolUseId = parentId;
              console.log(`[RoxAgent] CHILD REGISTERED (stream/single-parent): ${toolBlock.name} (${toolBlock.id}) under parent ${parentId}`);
            } else if (activeParentTools.size > 1) {
              // Multiple active parents - use SDK's parent_tool_use_id (authoritative source)
              ...
              if (sdkParentId && activeParentTools.has(sdkParentId)) {
                ...
                childToParent.set(toolBlock.id, sdkParentId);
                parentToolUseId = sdkParentId;
                console.log(`[RoxAgent] CHILD REGISTERED (stream/sdk-parent): ${toolBlock.name} (${toolBlock.id}) → Task (${sdkParentId})`);
              } else {
                // Fallback: FIFO if SDK doesn't provide parent
                const parentId = Array.from(activeParentTools)[0]!;
                parentToChildren.get(parentId)?.push(toolBlock.id);
                childToParent.set(toolBlock.id, parentId);
                parentToolUseId = parentId;
                console.log(`[RoxAgent] CHILD REGISTERED (stream/fifo-fallback): ${toolBlock.name} (${toolBlock.id}) → ${parentId} (sdk_parent=${sdkParentId})`);
              }
            }
            // else: no active parents - tool is top-level, no parent needed
      ...
      case 'user': {
        ...
        if (message.tool_use_result !== undefined) {
          ...
          if (toolUseId && activeParentTools.has(toolUseId)) {
            ...
            console.log(`[RoxAgent] RESULT MATCHING: parent=${toolUseId}, children.length=${children?.length || 0}`);
            if (children && children.length > 0) {
              const firstChild = children.shift()!; // Remove first child (FIFO)
              console.log(`[RoxAgent] MATCHED TO CHILD: ${firstChild}`);
              ...
              if (pendingChildId) {
                ...
                console.log(`[RoxAgent] MATCHED TO LATE CHILD: ${pendingChildId} (parent=${toolUseId})`);
                ...
                console.log(`[RoxAgent] NO CHILDREN LEFT - treating as parent's own result: ${toolUseId}`);
            ...
            if (pendingChildId) {
              ...
              console.log(`[RoxAgent] MATCHED TO ORPHANED CHILD: ${pendingChildId} (parent=${toolUseId})`);
          ...
          if (toolUseId) {
            ...
            if (toolUse?.name === 'Task' && toolUseId) {
              activeParentTools.delete(toolUseId);
              parentToChildren.delete(toolUseId);
              console.log(`[RoxAgent] PARENT CLEANED UP (task-result): ${toolUseId}`);
            }
      ...
      case 'tool_progress': {
        ...
        console.log(`[RoxAgent] tool_progress: tool=${progress.tool_name} (${progress.tool_use_id}), parent=${progress.parent_tool_use_id}, elapsed=${progress.elapsed_time_seconds}`);
        ...
        if (!emittedToolStarts.has(progress.tool_use_id)) {
          ...
          if (!isParentTool && progress.parent_tool_use_id && activeParentTools.has(progress.parent_tool_use_id)) {
            ...
            console.log(`[RoxAgent] CHILD REGISTERED (tool_progress/sdk): ${progress.tool_name} (${progress.tool_use_id}) → ${progress.parent_tool_use_id}`);
          }
      ...
      case 'result': {
        ...
        console.error(`[RoxAgent] result message: subtype=${message.subtype}, errors=${'errors' in message ? JSON.stringify((message as any).errors) : 'none'}`);
   ...
   * but not yet persisted to the Rox document
   */
