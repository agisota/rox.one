...
// System prompt for Rox Agent context
...
export class PiAgent extends BaseAgent {
  protected backendName = 'Rox Agents Backend';
  ...
  private async spawnSubprocess(): Promise<void> {
    ...
    // Keep the ROX_DEBUG-gated log for interactive dev work.
   ...
   *
   * OAuth tokens from Rox (Claude Max, ChatGPT Plus, Copilot) are passed as
   * api_key type because they function as bearer tokens that the Pi SDK's provider
   * modules use directly. The OAuth exchange happens on the Rox side; by the time
  ...
  private handleSubprocessEvent(event: Record<string, unknown>): void {
    ...
    // Feed them through PiEventAdapter to convert to Rox AgentEvents.
   ...
   * Uses the canonical registry from @rox-agent/session-tools-core.
   */
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const systemPrompt = getSystemPrompt(
        ...
        'Rox Agents Backend', // backendName
