...
import {
  CLI_DOMAIN_POLICIES,
  ROX_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES,
  ROX_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES,
...
export function qualifySkillName(
  ...
  // Extract the bare slug — strip any existing qualifier (e.g. "RoxAgentWS:commit" → "commit")
...
function buildCliDomainBlockMessage(namespace: CliDomainNamespace, context: string): string {
  ...
  return [
    `${context}`,
    `Use \`rox-agent ${namespace} ...\` instead.`,
...
export function getConfigCliRedirect(
  ...
  if (filePath && LABELS_BLOCKED_FILE_TOOLS.has(toolName)) {
    ...
    if (relativePath) {
      const labelsScopeMatch = ROX_AGENTS_CLI_WORKSPACE_SCOPE_ENTRIES.find(
...
/**
 * Block bash commands that operate on guarded config paths unless they use rox-agent commands.
...
export function getConfigDomainBashRedirect(
  ...
  if (/^rox-agent\s+(label|automation|source|skill)\b/.test(command)) {
    return null;
  }
  ...
  const bashGuardEntries: Array<{ namespace: CliDomainNamespace; scope: string }> = ROX_AGENTS_CLI_BASH_GUARD_SCOPE_ENTRIES
...
const BUILT_IN_MCP_SERVERS = new Set(['session', 'rox-agents-docs']);
...
export function runPreToolUseChecks(ctx: PreToolUseInput): PreToolUseCheckResult {
  ...
  // 5b. Config-domain Bash guard (block direct labels/automations path operations unless using rox-agent)
  if (FEATURE_FLAGS.roxAgentsCli && toolName === 'Bash') {
