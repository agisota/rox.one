...
function browserDocPath(): string {
  return resolve(join(homedir(), '.rox-agent', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts rox-agents-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('rox-agents-docs'));
      const result = manager.checkPrerequisites('mcp__rox-agents-docs__search');
      expect(result.allowed).toBe(true);
    });
