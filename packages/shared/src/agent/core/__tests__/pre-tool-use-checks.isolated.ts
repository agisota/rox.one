...
let mockRoxAgentsCliFlag = false;
mock.module('../../../feature-flags.ts', () => ({
  FEATURE_FLAGS: {
    get roxAgentsCli() {
      return mockRoxAgentsCliFlag;
    },
...
describe('runPreToolUseChecks', () => {
  ...
  describe('step 2: source blocking', () => {
    ...
    it('skips source check for built-in MCP servers (rox-agents-docs)', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'mcp__rox-agents-docs__search',
        input: {},
        activeSourceSlugs: [],
      }));
  ...
  describe('step 5: input transforms', () => {
    ...
    it('blocks direct label folder reads and suggests rox-agent label help when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('rox-agent label');
        expect(result.reason).toContain('rox-agent label --help');
        expect(result.reason).toContain('labels/');
      }
    });

    it('blocks direct label config writes and suggests rox-agent label help when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
    ...
    it('blocks direct automations config edits and suggests rox-agent automation commands when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('rox-agent automation');
        expect(result.reason).toContain('automations.json');
      }
    });

    it('blocks direct source config edits and suggests rox-agent source commands when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('rox-agent source');
        expect(result.reason).toContain('sources/linear/config.json');
      }
    });

    it('blocks direct skill file edits and suggests rox-agent skill commands when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
      mockDetectConfigFileType.mockImplementation(() => ({
        type: 'skill',
        slug: 'commit-helper',
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('rox-agent skill');
        expect(result.reason).toContain('skills/commit-helper/SKILL.md');
      }
    });

    it('blocks bash commands touching labels paths and points to rox-agent label --help when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
    ...
    it('allows bash rox-agent label commands through labels guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'rox-agent label list' },
        permissionMode: 'allow-all',
      }));
    ...
    it('blocks bash commands touching automations files and points to rox-agent automation --help when feature is enabled', () => {
      mockRoxAgentsCliFlag = true;
      ...
      expect(result.type).toBe('block');
      if (result.type === 'block') {
        expect(result.reason).toContain('rox-agent automation --help');
        expect(result.reason).toContain('rox-agent automation');
      }
    });

    it('allows bash rox-agent automation commands through config-domain bash guard', () => {
      const result = runPreToolUseChecks(createInput({
        toolName: 'Bash',
        input: { command: 'rox-agent automation list' },
        permissionMode: 'allow-all',
      }));
