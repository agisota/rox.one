...
class ModeManager {
  ...
  setPermissionMode(sessionId: string, mode: PermissionMode): void {
    ...
    // Notify callbacks (for RoxAgent internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string): string | null {
  ...
  if (normalizedTarget.includes('/.rox-agent/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans folder, not the workspace root.';
  }

  // Case: Writing outside .rox-agent entirely
  if (!normalizedTarget.includes('/.rox-agent/')) {
    return 'Hint: Plans must be written to the .rox-agent session plans folder.';
  }
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__preferences__') || toolName.startsWith('mcp__rox-agents-docs__')) {
      return { allowed: true };
    }
