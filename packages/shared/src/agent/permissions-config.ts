 ...
 * - Per-source: ~/.rox-agent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.rox-agent/permissions/
 * Reads env var dynamically so tests can override via ROX_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.ROX_CONFIG_DIR || join(homedir(), '.rox-agent');
...
/**
 * Load default permissions from ~/.rox-agent/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.roxAgentsCli && pattern.startsWith('^rox-agent\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.rox-agent/permissions/default.json)
   ...
   * These come from ~/.rox-agent/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping rox-agent bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
