...
export class CodexAgent extends BaseAgent {
  ...
  private setupClientEventHandlers(): void {
    ...
    // ROX AGENTS: PreToolUse hook - intercept ALL tools before execution
  ...
  /**
   * ROX AGENTS: Handle PreToolUse hook request.
   *
      ...
      if (this.codexThreadId) {
        ...
        try {
          await client.threadResume({
            ...
            sandbox: null,
            config: null,
            // Inject Rox Agent system prompt on resume (mini or full)
          ...
          const response = await client.threadStart({
            ...
            // Inject Rox Agent system prompt (mini or full)
            baseInstructions: miniConfig.enabled
              ? this.getMiniSystemPrompt()
              : getSystemPrompt(
                  undefined, // preferences formatted fresh
    ...
    if (threadId) {
      try {
        ...
        await client.threadResume({
          ...
          sandbox: null,
          config: null,
          // Re-inject Rox Agent system prompt after reconnect
          baseInstructions: miniConfig.enabled
            ? this.getMiniSystemPrompt()
            : getSystemPrompt(
                undefined, // preferences formatted fresh
