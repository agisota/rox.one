...
const originalCliFlag = process.env.ROX_FEATURE_ROX_AGENTS_CLI
    ...
    JSON.stringify(
      {
        version: '2026-03-07',
        allowedBashPatterns: [
          { pattern: '^rox-agent\\s+label\\s+list\\b', comment: 'rox-agent label read-only operations' },
...
afterEach(() => {
  ...
  if (originalCliFlag === undefined) delete process.env.ROX_FEATURE_ROX_AGENTS_CLI
  else process.env.ROX_FEATURE_ROX_AGENTS_CLI = originalCliFlag
})

describe('permissions config rox-agents-cli feature flag', () => {
  it('skips compiling rox-agent bash allowlist patterns when feature is disabled', () => {
    const tempConfigDir = mkdtempSync(join(tmpdir(), 'rox-permissions-'))
    try {
      ...
      expect(sources.some(source => source.startsWith('^rox-agent\\s'))).toBe(false)
  ...
  it('compiles rox-agent bash allowlist patterns when feature is enabled', () => {
    const tempConfigDir = mkdtempSync(join(tmpdir(), 'rox-permissions-'))
    try {
      process.env.ROX_CONFIG_DIR = tempConfigDir
      ...
      const sources = merged.readOnlyBashPatterns.map(p => p.source)
      expect(sources).toContain('^rox-agent\\s+label\\s+list\\b')
      expect(sources).toContain('^rg\\b')
    } finally {
      rmSync(tempConfigDir, { recursive: true, force: true })
    }
