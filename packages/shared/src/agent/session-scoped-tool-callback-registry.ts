...
import type { AuthRequest } from '@rox-agent/session-tools-core';
...
export interface SessionScopedToolCallbacks {
  ...
  getSessionInfoFn?: (sessionId?: string) => import('@rox-agent/session-tools-core').SessionInfo | null;
  /** List sessions in the workspace with pagination. */
  listSessionsFn?: (options?: import('@rox-agent/session-tools-core').ListSessionsOptions) => import('@rox-agent/session-tools-core').ListSessionsResult;
  /** Resolve label display names to IDs. */
  resolveLabelsFn?: (labels: string[]) => import('@rox-agent/session-tools-core').ResolvedLabelsResult;
  /** Resolve a status display name to its ID. */
  resolveStatusFn?: (status: string) => import('@rox-agent/session-tools-core').ResolvedStatusResult;
}
