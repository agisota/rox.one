...
import type { AuthRequest } from '@rox-one/session-tools-core';
...
export interface SessionScopedToolCallbacks {
  ...
  getSessionInfoFn?: (sessionId?: string) => import('@rox-one/session-tools-core').SessionInfo | null;
  /** List sessions in the workspace with pagination. */
  listSessionsFn?: (options?: import('@rox-one/session-tools-core').ListSessionsOptions) => import('@rox-one/session-tools-core').ListSessionsResult;
  /** Resolve label display names to IDs. */
  resolveLabelsFn?: (labels: string[]) => import('@rox-one/session-tools-core').ResolvedLabelsResult;
  /** Resolve a status display name to its ID. */
  resolveStatusFn?: (status: string) => import('@rox-one/session-tools-core').ResolvedStatusResult;
