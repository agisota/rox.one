 ...
 * @rox-agent/session-tools-core for use with the Claude SDK.
 *
 * Tools included:
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
  ...
  config_validate: `Validate Rox Agent configuration files.
...
- \`statuses\`: Validates ~/.rox-agent/workspaces/{workspace}/statuses/config.json
- \`preferences\`: Validates ~/.rox-agent/preferences.json
- \`permissions\`: Validates permissions.json files
- \`tool-icons\`: Validates ~/.rox-agent/tool-icons/tool-icons.json
...
async function handleTransformData(
  ...
  const tempScript = join(tmpdir(), `rox-transform-${sessionId}-${Date.now()}${ext}`);
