 ...
 * See ~/.rox-agent/docs/ for config format documentation.
 */
...
import { renderMermaid } from '@rox-agent/mermaid';
 ...
 * Called by RoxAgent when initializing.
 */
...
export function createConfigValidateTool(sessionId: string, workspaceRootPath: string) {
  return tool(
    'config_validate',
    `Validate Rox Agent configuration files.
...
**Targets:**
- \`config\`: Validates ~/.rox-agent/config.json (workspaces, model, settings)
- \`sources\`: Validates all sources in ~/.rox-agent/workspaces/{workspace}/sources/*/config.json
- \`statuses\`: Validates ~/.rox-agent/workspaces/{workspace}/statuses/config.json (workflow states)
- \`preferences\`: Validates ~/.rox-agent/preferences.json (user preferences)
- \`permissions\`: Validates permissions.json files (workspace, source, and app-level default)
- \`tool-icons\`: Validates ~/.rox-agent/tool-icons/tool-icons.json (CLI tool icon mappings)
      ...
      try {
        ...
        if (!source) {
          return {
            content: [{
              type: 'text' as const,
              text: `Source '${args.sourceSlug}' not found. Check ~/.rox-agent/workspaces/{workspace}/sources/ for available sources.`,
            }],
            isError: true,
          };
        }
 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.rox-agent/workspaces/xxx)
 */
