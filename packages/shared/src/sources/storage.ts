 ...
 * (like rox-agents-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only rox-agents-docs is a builtin source
      if (slug === 'rox-agents-docs') {
 ...
 * Built-in sources (like rox-agents-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search rox-agents-docs MCP for service-specific guidance)
