#!/usr/bin/env bun
/**
 * Debug script to print the full Rox Agent system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    ROX AGENT SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. Rox Agent Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.rox-agent/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@rox-agent/electron/main.log' },
  '/Users/example/.rox-agent/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.rox-agent/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by RoxAgent.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Rox Agent additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Rox Agent Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/rox-agent.ts       ${colors.dim}// User message building${colors.reset}
