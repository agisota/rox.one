...
export class CredentialManager {
  ...
  /** Get Rox OAuth token */
  async getRoxOAuth(): Promise<string | null> {
    const cred = await this.get({ type: 'rox_oauth' });
    return cred?.value || null;
  }

  /** Set Rox OAuth token */
  async setRoxOAuth(token: string): Promise<void> {
    await this.set({ type: 'rox_oauth' }, { value: token });
  }
