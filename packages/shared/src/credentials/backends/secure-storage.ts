 ...
 *
 * Stores credentials in an encrypted file at ~/.rox-agent/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.rox-agent');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const machineId = createHash('sha256')
      ...
      .update(homedir())
      .update('rox-agent-v1') // App-specific constant
      .digest();
