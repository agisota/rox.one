...
describe('buildOAuthDeeplinkUrl', () => {
  ...
  it('uses custom deeplink scheme for multi-instance', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'roxagents1',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('roxagents1://allSessions/session/test-session');
  });
  ...
  it('constructs correct URL format for instance 2', () => {
    const ctx: OAuthSessionContext = {
      ...
      deeplinkScheme: 'roxagents2',
    };
    expect(buildOAuthDeeplinkUrl(ctx)).toBe('roxagents2://allSessions/session/260111-bold-moon');
  });
});
