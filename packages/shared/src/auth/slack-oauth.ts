...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = `https://agents.rox.do/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://agents.rox.do/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://agents.rox.do/auth/slack/callback?port=${port}`;
