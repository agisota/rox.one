 ...
 * Default location: ~/.rox-agent/workspaces/
 */
...
/**
 * Get the default workspaces directory (~/.rox-agent/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.rox-agent/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.rox-agent/workspaces/
 */
 ...
 * Resolution order: ENV (ROX_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.ROX_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `rox-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
