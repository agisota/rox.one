import { RoxAgent, type RoxAgentConfig, type PermissionMode, type SdkMcpServerConfig } from '../agent/rox-agent.ts';
 ...
 * - RoxMcpClient for MCP connections
 * - RoxAgent for query execution
 *
...
export class HeadlessRunner {
  ...
  private agent: RoxAgent | null = null;
  ...
  async *runStreaming(): AsyncGenerator<HeadlessEvent> {
    try {
      ...
      // 2. Create RoxAgent with headless callbacks
...
${this.config.prompt}
  ...
  /**
   * Create RoxAgent with headless callbacks for permissions and questions.
   */
  private async createAgent(): Promise<void> {
    ...
    const agentConfig: RoxAgentConfig = {
    ...
    this.agent = new RoxAgent(agentConfig);
    ...
    if (this.config.sessionId && this.workspaceRootPath) {
      ...
      if (session.sdkSessionId) {
        debug('[HeadlessRunner] Resuming session (--session) - rox:', session.id, 'sdk:', session.sdkSessionId);
        this.agent.setSessionId(session.sdkSessionId);
      } else {
        debug('[HeadlessRunner] New session created (--session) - rox:', session.id, 'sdk: none (will be saved after run)');
      ...
      if (sessions.length > 0 && sessions[0]) {
        ...
        if (sessions[0].sdkSessionId) {
          debug('[HeadlessRunner] Resuming last session (--session-resume) - rox:', sessions[0].id, 'sdk:', sessions[0].sdkSessionId);
          this.agent.setSessionId(sessions[0].sdkSessionId);
        } else {
          debug('[HeadlessRunner] Last session has no SDK session (--session-resume) - rox:', sessions[0].id, 'sdk: none');
        }
      } else {
      ...
      debug('[HeadlessRunner] Fresh session (default headless mode) - no rox session, no sdk session');
    }
  }
  ...
  private async cleanup(): Promise<void> {
    ...
    if (this.sessionIdToUpdate && this.agent && this.workspaceRootPath) {
      ...
      if (sdkSessionId) {
        debug('[HeadlessRunner] Saving session - rox:', this.sessionIdToUpdate, 'sdk:', sdkSessionId);
