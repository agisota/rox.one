...
export function isDeveloperFeedbackEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('ROX_FEATURE_DEVELOPER_FEEDBACK'));
...
export function isRoxAgentsCliEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('ROX_FEATURE_ROX_AGENTS_CLI'));
  if (override !== undefined) return override;
  return false;
}
 ...
 *
 * Defaults to disabled. Override with ROX_FEATURE_EMBEDDED_SERVER=1|0.
 */
export function isEmbeddedServerEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('ROX_FEATURE_EMBEDDED_SERVER'));
  if (override !== undefined) return override;
  return false;
}
...
export const FEATURE_FLAGS = {
   ...
   * Override with ROX_FEATURE_DEVELOPER_FEEDBACK=1|0.
   */
