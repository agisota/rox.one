...
export class McpPoolServer {
   ...
   * Tools from pool use `mcp__rox__search_spaces` naming internally.
   ...
   *   pool internal: mcp__rox__search_spaces
   *   exposed here:  rox__search_spaces
   *   Codex sees:    mcp__sources__rox__search_spaces
   */
  private createMcpServer(): Server {
    const server = new Server(
      { name: 'rox-pool-proxy', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );
