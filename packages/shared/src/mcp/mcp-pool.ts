...
import { RoxMcpClient, type McpClientConfig, type PoolClient } from './client.ts';
...
/**
 * Convert SdkMcpServerConfig (used by backend types) to RoxMcpClient config.
 */
...
export class McpClientPool {
  ...
  protected async registerClient(slug: string, client: PoolClient): Promise<void> {
    // listTools() triggers connect() internally for both RoxMcpClient and ApiSourcePoolClient
  ...
  async connect(slug: string, config: SdkMcpServerConfig): Promise<void> {
    ...
    await this.registerClient(slug, new RoxMcpClient(clientConfig));
