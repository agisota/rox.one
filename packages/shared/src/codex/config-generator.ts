 ...
 *
 * Generates config.toml files from Rox Agent sources for per-session Codex configuration.
...
export function generateCodexConfig(options: CodexConfigGeneratorOptions): CodexConfigResult {
  ...
  // Header comment
  sections.push('# Generated by Rox Agent - DO NOT EDIT');
  ...
  // Rox Agents documentation - public Mintlify MCP server, no auth needed.
  // Provides SearchRoxAgents tool for finding source setup guides.
  // This matches the rox-agents-docs server in Claude's SDK options.
  sections.push('# Rox Agents documentation (source setup guides, API references)');
  sections.push('[mcp_servers.rox-agents-docs]');
  sections.push('url = "https://agents.rox.do/docs/mcp"');
