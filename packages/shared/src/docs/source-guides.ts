 ...
 *
 * Guides are stored at ~/.rox-agent/docs/source-guides/ and copied on first run.
 */
...
export function getSourceGuideForDomain(domain: string): ParsedSourceGuide | null {
  ...
  for (const [filename, content] of Object.entries(BUNDLED_SOURCE_GUIDES)) {
    ...
    // Check filename match (e.g., "rox.do.md" matches "rox.do")
...
Everything in Rox is scoped to a **Space**. Users may have multiple spaces, but you can only act within the current space. Spaces can be shared, but are typically used by one person.
...
Documents are the core of Rox. Each document has a unique ID.
...
### Configuration Notes
- **MCP URL**: \`https://mcp.rox.do/my/mcp\`
- Rox MCP uses OAuth authentication
...
GitHub's MCP server requires OAuth with a **pre-registered client app**. Rox Agent is NOT registered with GitHub, so OAuth authentication WILL fail. This is expected behavior - do NOT troubleshoot or search for solutions.
