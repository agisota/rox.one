...
Rox Agent uses a 6-color theme system with cascading configuration:
- **App-level theme**: \`~/.rox-agent/theme.json\` - Global defaults
- **Workspace-level theme**: \`~/.rox-agent/workspaces/{id}/theme.json\` - Per-workspace overrides
...
Create \`~/.rox-agent/workspaces/{id}/theme.json\`:
\`\`\`json
{
...
1. **App theme** (\`~/.rox-agent/theme.json\`) sets global defaults
...
\`\`\`bash
# Create or edit ~/.rox-agent/theme.json
\`\`\`
...
# ~/.rox-agent/workspaces/{workspaceId}/theme.json
\`\`\`
