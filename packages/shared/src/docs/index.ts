 ...
 *
 * Docs are stored at ~/.rox-agent/docs/ and copied on first run.
 */
...
const CONFIG_DIR = join(homedir(), '.rox-agent');
...
export const DOC_REFS = {
  sources: '~/.rox-agent/docs/sources.md',
  permissions: '~/.rox-agent/docs/permissions.md',
  skills: '~/.rox-agent/docs/skills.md',
  themes: '~/.rox-agent/docs/themes.md',
  statuses: '~/.rox-agent/docs/statuses.md',
  docsDir: '~/.rox-agent/docs/',
} as const;
...
This guide explains how to configure sources (MCP servers, APIs, local filesystems) in Rox Agent.
...
**Before doing anything else**, search for a specialized guide using the rox-agents-docs MCP:

\`\`\`
mcp__rox-agents-docs__SearchRoxAgents({ query: "{service} source setup" })
\`\`\`

**Available guides:** GitHub, Linear, Slack, Gmail, Google Calendar, Google Drive, Google Docs, Google Sheets, Outlook, Microsoft Calendar, Teams, SharePoint, Rox, Filesystem, Brave Search, Memory
...
- "To find issues in the Rox iOS project: ..."
...
Agent: [FIRST: Searches for Linear guide]
       mcp__rox-agents-docs__SearchRoxAgents({ query: "linear source setup" })
...
User: Issue tracking for my iOS team, project called "Rox iOS"
...
Agent: [Creates config.json with:
        ...
        - tagline: "Issue tracking for the Rox iOS team"
        - OAuth authentication]
Agent: [Writes guide.md mentioning "Rox iOS" project and iOS team context]
...
Agent: Linear is now set up! I've configured it to focus on the "Rox iOS" project.
You can use it to:
- Search and view issues in Rox iOS
...
- \`~/.rox-agent/workspaces/{workspaceId}/sources/{sourceSlug}/\`
...
\`~/.rox-agent/provider-domains.json\`
...
1. Create the source folder:
   \`\`\`bash
   mkdir -p ~/.rox-agent/workspaces/{ws}/sources/my-source
   \`\`\`
...
This guide explains how to create and configure skills in Rox Agent.
...
Rox Agent uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Rox Agent
...
1. **Workspace skill checked first** - If \`~/.rox-agent/workspaces/{id}/skills/commit/SKILL.md\` exists, it's used
...
\`\`\`
~/.rox-agent/workspaces/{workspaceId}/skills/{slug}/
...
\`\`\`bash
mkdir -p ~/.rox-agent/workspaces/{ws}/skills/my-skill
\`\`\`
...
1. Create \`~/.rox-agent/workspaces/{ws}/skills/commit/SKILL.md\`
...
- Workspace: \`~/.rox-agent/workspaces/{slug}/permissions.json\`
- Source: \`~/.rox-agent/workspaces/{slug}/sources/{source}/permissions.json\`
...
This guide explains how to customize the visual theme of Rox Agent.

## Overview

Rox Agent uses a 6-color theme system. You can override specific colors or install preset themes with complete visual styles.

- **Theme overrides**: \`~/.rox-agent/theme.json\` - Override specific colors
- **Preset themes**: \`~/.rox-agent/themes/{name}.json\` - Complete theme packages
...
Create \`~/.rox-agent/theme.json\` to override specific colors:
...
Preset themes are complete theme packages stored at \`~/.rox-agent/themes/\`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to \`~/.rox-agent/themes/{name}.json\`
...
1. Create \`~/.rox-agent/theme.json\` for overrides or \`~/.rox-agent/themes/{name}.json\` for a preset
...
- Check file is in correct location (\`~/.rox-agent/theme.json\` for overrides, \`~/.rox-agent/themes/\` for presets)
