...
export class PostgresAccountStore implements AccountStore {
  ...
  async migrate(): Promise<void> {
    await this.pool.query(`
      ...
      ALTER TABLE craft_users ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'pending_email_verification';
      ALTER TABLE craft_users ADD COLUMN IF NOT EXISTS email_verified_at TIMESTAMPTZ;

      CREATE TABLE IF NOT EXISTS craft_account_sessions (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL REFERENCES craft_users(id) ON DELETE CASCADE,
        user_agent TEXT,
        ip_address TEXT,
      ...
      ALTER TABLE craft_account_sessions ADD COLUMN IF NOT EXISTS auth_method TEXT NOT NULL DEFAULT 'password';

      CREATE INDEX IF NOT EXISTS craft_account_sessions_user_id_idx ON craft_account_sessions(user_id);
      CREATE INDEX IF NOT EXISTS craft_account_sessions_active_idx ON craft_account_sessions(user_id, expires_at) WHERE revoked_at IS NULL;

      CREATE TABLE IF NOT EXISTS craft_account_email_tokens (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL REFERENCES craft_users(id) ON DELETE CASCADE,
      ...
      CREATE INDEX IF NOT EXISTS craft_account_email_tokens_lookup_idx
        ON craft_account_email_tokens(purpose, token_hash, expires_at)
        WHERE consumed_at IS NULL;
      CREATE INDEX IF NOT EXISTS craft_account_email_tokens_user_id_idx
        ON craft_account_email_tokens(user_id);

      CREATE TABLE IF NOT EXISTS craft_workspace_owners (
        user_id TEXT NOT NULL REFERENCES craft_users(id) ON DELETE CASCADE,
      ...
      CREATE INDEX IF NOT EXISTS craft_workspace_owners_workspace_id_idx ON craft_workspace_owners(workspace_id);
    `)
  }

  async getUserCount(): Promise<number> {
    const result = await this.pool.query<{ count: string }>('SELECT COUNT(*) AS count FROM craft_users')
  ...
  async createUser(input: CreateUserInput): Promise<PublicUser> {
    ...
    try {
      ...
      await client.query("SELECT pg_advisory_xact_lock(hashtext('craft_account_first_admin')::bigint)")
      const countResult = await client.query<{ count: string }>('SELECT COUNT(*) AS count FROM craft_users')
      const role: AccountRole = Number(countResult.rows[0]?.count ?? 0) === 0 ? 'admin' : 'user'
      const result = await client.query<UserRow>(
        `INSERT INTO craft_users (id, email, password_hash, display_name, role, status)
  ...
  async createSession(input: CreateAccountSessionInput): Promise<AccountSession> {
    ...
    const result = await this.pool.query<SessionRow>(
      `INSERT INTO craft_account_sessions (id, user_id, user_agent, ip_address, auth_method, expires_at)
       VALUES ($1, $2, $3, $4, $5, $6)
  ...
  async getSessionIdentity(sessionId: string): Promise<SessionIdentity | null> {
    const result = await this.pool.query<{
      ...
      `SELECT s.id AS session_id, u.id AS user_id, u.email, u.display_name, u.role
       FROM craft_account_sessions s
       JOIN craft_users u ON u.id = s.user_id
  ...
  async revokeUserSessions(userId: string): Promise<void> {
    await this.pool.query(
      `UPDATE craft_account_sessions SET revoked_at = now() WHERE user_id = $1 AND revoked_at IS NULL`,
      [userId],
    )
  }
  ...
  async getUser(userId: string): Promise<PublicUser | null> {
    const result = await this.pool.query<UserRow>(
      `SELECT id, email, password_hash, display_name, role, status, email_verified_at, created_at, updated_at FROM craft_users WHERE id = $1`,
      [userId],
    )
    return result.rows[0] ? toPublicUser(result.rows[0]) : null
  }
  ...
  async createEmailToken(input: CreateEmailTokenInput): Promise<{ rawToken: string; expiresAt: string }> {
    ...
    await this.pool.query(
      `INSERT INTO craft_account_email_tokens (id, user_id, purpose, token_hash, expires_at)
       VALUES ($1, $2, $3, $4, $5)`,
  ...
  async consumeEmailToken(purpose: EmailTokenPurpose, rawToken: string): Promise<PublicUser | null> {
    const result = await this.pool.query<{ user_id: string }>(
      `UPDATE craft_account_email_tokens
       SET consumed_at = now()
       WHERE id = (
         SELECT id
         FROM craft_account_email_tokens
         WHERE purpose = $1
           AND token_hash = $2
  ...
  async setPassword(userId: string, newPassword: string): Promise<void> {
    ...
    await this.pool.query(
      `UPDATE craft_users SET password_hash = $2, updated_at = now() WHERE id = $1`,
      [userId, passwordHash],
    )
  }
  ...
  async grantWorkspaceOwner(userId: string, workspaceId: string): Promise<void> {
    await this.pool.query(
      `INSERT INTO craft_workspace_owners (user_id, workspace_id)
       VALUES ($1, $2)
  ...
  async isWorkspaceOwner(userId: string, workspaceId: string): Promise<boolean> {
    const result = await this.pool.query(
      `SELECT 1 FROM craft_workspace_owners WHERE user_id = $1 AND workspace_id = $2 LIMIT 1`,
      [userId, workspaceId],
    )
  ...
  async listWorkspaceIds(userId: string): Promise<string[]> {
    const result = await this.pool.query<{ workspace_id: string }>(
      `SELECT workspace_id FROM craft_workspace_owners WHERE user_id = $1 ORDER BY created_at ASC`,
      [userId],
    )
