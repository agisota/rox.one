...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle rox:// internally)
    ...
    try {
      ...
      if (parsed.protocol === 'rox:' || parsed.protocol === 'roxagents:') {
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing rox:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for rox:// URL')
