...
import { validateFilePath } from '@rox-agent/server-core/handlers'
    ...
    try {
      ...
      if (parsed.protocol === 'roxagents:') {
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing roxagents:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for roxagents:// URL')
      ...
      if (!['http:', 'https:', 'mailto:', 'roxdocs:'].includes(parsed.protocol)) {
        throw new Error('Only http, https, mailto, roxdocs, roxagents URLs are allowed')
      }
