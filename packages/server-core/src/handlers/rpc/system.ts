...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      // roxagents:// URLs require the GUI deep-link handler (Electron only)
      if (parsed.protocol === 'roxagents:') {
        deps.platform.logger.info('[OPEN_URL] roxagents:// URLs require GUI deep-link handler — skipping in core')
        return
      }
