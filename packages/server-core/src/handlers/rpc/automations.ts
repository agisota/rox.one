...
export function registerAutomationsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload: import('@rox-agent/shared/protocol').TestAutomationPayload) => {
    const workspace = getWorkspaceByNameOrId(payload.workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const results: import('@rox-agent/shared/protocol').TestAutomationActionResult[] = []
    const { parsePromptReferences } = await import('@rox-agent/shared/automations')
    const { executeWebhookRequest, createWebhookHistoryEntry, createPromptHistoryEntry } = await import('@rox-agent/shared/automations/webhook-utils')
    ...
    for (const action of payload.actions) {
      ...
      if (action.type === 'webhook') {
        ...
        const result = await executeWebhookRequest(action as import('@rox-agent/shared/automations').WebhookAction)
    ...
    return { actions: results } satisfies import('@rox-agent/shared/protocol').TestAutomationResult
  })
  ...
  server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId: string, automationId: string, eventName: string) => {
    ...
    const { resolveAutomationsConfigPath } = await import('@rox-agent/shared/automations/resolve-config-path')
    const configPath = resolveAutomationsConfigPath(workspace.rootPath)
    const raw = await readFile(configPath, 'utf-8')
    ...
    const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@rox-agent/shared/automations/webhook-utils')
    const results = await Promise.all(
      webhookActions.map(a => executeWebhookRequest(a as unknown as import('@rox-agent/shared/automations').WebhookAction))
    )
