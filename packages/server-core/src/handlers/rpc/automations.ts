...
import { appendAutomationHistoryEntry } from '@rox-one/shared/automations/history-store'
import { AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER } from '@rox-one/shared/automations/constants'
import type { RpcServer } from '@rox-one/server-core/transport'
import type { HandlerDeps } from '../handler-deps'
import { parseId, parseSafeString } from './_validators'

// History file name — matches AUTOMATIONS_HISTORY_FILE from @rox-one/shared/automations/constants
...
async function withAutomationMatcher(workspaceId: string, eventName: string, matcherIndex: number, mutate: (matchers: Record<string, unknown>[], index: number, config: AutomationsConfigJson, genId: () => string) => void) {
  ...
  await withConfigMutex(workspace.rootPath, async () => {
    const { resolveAutomationsConfigPath, generateShortId } = await import('@rox-one/shared/automations/resolve-config-path')
...
export function registerAutomationsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.automations.GET, async (_ctx, workspaceId: string) => {
    ...
    try {
      const { resolveAutomationsConfigPath } = await import('@rox-one/shared/automations/resolve-config-path')
      const configPath = resolveAutomationsConfigPath(workspace.rootPath)
  ...
  server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload: import('@rox-one/shared/protocol').TestAutomationPayload) => {
    parseId('payload.workspaceId', payload?.workspaceId)
    const workspace = getWorkspaceByNameOrId(payload.workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const results: import('@rox-one/shared/protocol').TestAutomationActionResult[] = []
    const { parsePromptReferences } = await import('@rox-one/shared/automations')
    const { executeWebhookRequest, createWebhookHistoryEntry, createPromptHistoryEntry } = await import('@rox-one/shared/automations/webhook-utils')
    ...
    for (const action of payload.actions) {
      ...
      if (action.type === 'webhook') {
        ...
        const result = await executeWebhookRequest(action as import('@rox-one/shared/automations').WebhookAction)
    ...
    return { actions: results } satisfies import('@rox-one/shared/protocol').TestAutomationResult
  })
  ...
  server.handle(RPC_CHANNELS.automations.SET_ENABLED, async (_ctx, workspaceId: string, eventName: string, matcherIndex: number, enabled: boolean) => {
    parseId('workspaceId', workspaceId)
    parseSafeString('eventName', eventName, 256)
  ...
  server.handle(RPC_CHANNELS.automations.GET_HISTORY, async (_ctx, workspaceId: string, automationId: string, limit = AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER) => {
    parseId('workspaceId', workspaceId)
    parseId('automationId', automationId)
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')
  ...
  server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId: string, automationId: string, eventName: string) => {
    ...
    const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@rox-one/shared/automations/webhook-utils')
    const results = await Promise.all(
      webhookActions.map(a => executeWebhookRequest(a as unknown as import('@rox-one/shared/automations').WebhookAction))
    )
