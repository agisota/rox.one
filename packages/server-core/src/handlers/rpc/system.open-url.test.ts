...
import type { RpcServer, HandlerFn, RequestContext } from '@rox-agent/server-core/transport'
...
describe('registerSystemCoreHandlers OPEN_URL', () => {
  ...
  it('rejects unsupported protocols', async () => {
    ...
    await expect(openUrl(ctx, 'file:///tmp/test.txt')).rejects.toThrow(
      'Failed to open URL: Only http, https, mailto, roxdocs, roxagents URLs are allowed'
    )
  })
})
