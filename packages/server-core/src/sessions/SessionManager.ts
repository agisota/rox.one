...
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns, generateConversationSummary } from '@rox-agent/shared/agent'
import {
...
import { getLlmConnection, getLlmConnections, getDefaultLlmConnection, getDefaultThinkingLevel, resetManagedAnthropicAuthEnvVars } from '@rox-agent/shared/config'
...
import { listLabels, loadLabelConfig } from '@rox-agent/shared/labels/storage'
import { extractLabelId } from '@rox-agent/shared/labels'
import { ensureLabelsExist } from '@rox-agent/shared/labels/crud'
import { flattenLabels } from '@rox-agent/shared/labels/tree'
...
export class SessionManager implements ISessionManager {
  ...
  private async generateRemoteTransferSummary(managed: ManagedSession): Promise<string | null> {
    ...
    const envOverrides: Record<string, string> = {
      ROX_WORKSPACE_PATH: workspaceRootPath,
      ...(miniModel ? { ANTHROPIC_DEFAULT_HAIKU_MODEL: miniModel } : {}),
    }
