...
import type { ISessionManager, IBrowserPaneManager } from '@rox-agent/server-core/handlers'
import { createScopedLogger, CONSOLE_LOGGER, type PlatformServices, type Logger } from '@rox-agent/server-core/runtime'
...
import { type AgentEvent, setPermissionMode, hydratePreviousPermissionMode, getPermissionModeDiagnostics, type PermissionMode, unregisterSessionScopedToolCallbacks, mergeSessionScopedToolCallbacks, AbortReason, type AuthRequest, type AuthResult, type CredentialAuthRequest, type BrowserPaneFns } from '@rox-agent/shared/agent'
import {
...
import { getLlmConnection, getDefaultLlmConnection, getDefaultThinkingLevel } from '@rox-agent/shared/config'
...
import { loadWorkspaceSources, loadAllSources, getSourcesBySlugs, isSourceUsable, type LoadedSource, type McpServerConfig, getSourcesNeedingAuth, getSourceCredentialManager, getSourceServerBuilder, type SourceWithCredential, isApiOAuthProvider, SERVER_BUILD_ERRORS, TokenRefreshManager, createTokenGetter } from '@rox-agent/shared/sources'
import { ConfigWatcher, type ConfigWatcherCallbacks } from '@rox-agent/shared/config'
import { getValidClaudeOAuthToken } from '@rox-agent/shared/auth'
import { resolveAuthEnvVars } from '@rox-agent/shared/config'
import { toolMetadataStore, getLastApiError } from '@rox-agent/shared/interceptor'
...
import { type Session, type SessionEvent, type FileAttachment, type SendMessageOptions, type UnreadSummary, RPC_CHANNELS, generateMessageId } from '@rox-agent/shared/protocol'
import { messageToStored, storedToMessage, type Message, type StoredAttachment, type ToolDisplayMeta } from '@rox-agent/core/types'
import { formatPathsToRelative, formatToolInputPaths, perf, encodeIconToDataUrlAsync, getEmojiIcon, resetSummarizationClient, resolveToolIcon, readFileAttachment, selectSpreadMessages } from '@rox-agent/shared/utils'
...
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL } from '@rox-agent/shared/agent/thinking-levels'
import { evaluateAutoLabels } from '@rox-agent/shared/labels/auto'
import { listLabels } from '@rox-agent/shared/labels/storage'
import { extractLabelId } from '@rox-agent/shared/labels'
import { ensureLabelsExist } from '@rox-agent/shared/labels/crud'
import { AutomationSystem, AUTOMATIONS_HISTORY_FILE, type AutomationSystemMetadataSnapshot } from '@rox-agent/shared/automations'
...
import { resizeImageForAPI, resizeIconBuffer } from '@rox-agent/server-core/services'
...
async function applyBridgeUpdates(
  ...
  mcpServers: Record<string, import('@rox-agent/shared/agent/backend').SdkMcpServerConfig>,
  sessionId: string,
...
async function getBrowserToolIconDataUrl(): Promise<string | undefined> {
  ...
  try {
    const iconCandidates = [
      ...
      // Dev fallback (before sync to ~/.rox-agent/tool-icons)
...
async function resolveToolDisplayMeta(
  ...
  // and resolves their brand icon from ~/.rox-agent/tool-icons/
...
interface ManagedSession {
  ...
  // pendingTextParent) has been removed. RoxAgent now provides parentToolUseId
...
export class SessionManager implements ISessionManager {
  ...
  private async reloadSessionSources(managed: ManagedSession): Promise<void> {
    ...
    // Reload all sources from disk (rox-agents-docs is always available as MCP server)
   ...
   * See: https://github.com/lukilabs/rox-agents-oss/issues/39
   */
  /**
   * Reinitialize authentication environment variables.
   *
  ...
  async createSession(workspaceId: string, options?: import('@rox-agent/shared/protocol').CreateSessionOptions): Promise<Session> {
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    if (!managed.agent) {
      ...
      // The SDK subprocess reads ROX_SESSION_DIR to write tool-metadata.json;
      ...
      process.env.ROX_SESSION_DIR = sessionDirForMetadata
      ...
      const envOverrides: Record<string, string> = {
        ROX_WORKSPACE_PATH: managed.workspace.rootPath,
      }
      ...
      // The legacy onCredentialRequest callback has been removed from RoxAgent
   ...
   */
  async shareToViewer(sessionId: string): Promise<import('@rox-agent/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
   ...
   */
  async updateShare(sessionId: string): Promise<import('@rox-agent/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
   ...
   */
  async revokeShare(sessionId: string): Promise<import('@rox-agent/shared/protocol').ShareResult> {
    const managed = this.sessions.get(sessionId)
    if (!managed) {
      return { success: false, error: 'Session not found' }
    }
  ...
  async sendMessage(sessionId: string, message: string, attachments?: FileAttachment[], storedAttachments?: StoredAttachment[], options?: SendMessageOptions, existingMessageId?: string, _isAuthRetry?: boolean): Promise<void> {
    ...
    try {
      ...
      // in the rawText, and canUseTool in rox-agent.ts provides a fallback
  ...
  private async processEvent(managed: ManagedSession, event: AgentEvent): Promise<void> {
    ...
    switch (event.type) {
      ...
      case 'tool_start': {
        ...
        // Use parentToolUseId directly from the event — RoxAgent resolves this
      ...
      case 'tool_result': {
        // toolName comes directly from RoxAgent (resolved via ToolIndex)
        ...
        // parentToolUseId comes from RoxAgent (SDK-authoritative) or existing message
      ...
      case 'complete':
        // Complete event from RoxAgent - accumulate usage from this turn
