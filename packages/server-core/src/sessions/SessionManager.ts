...
import { getLlmConnection, getDefaultLlmConnection, getDefaultThinkingLevel, resetManagedAnthropicAuthEnvVars } from '@rox-agent/shared/config'
...
import { type ThinkingLevel, DEFAULT_THINKING_LEVEL, normalizeThinkingLevel } from '@rox-agent/shared/agent/thinking-levels'
  ...
  private async getOrCreateAgent(managed: ManagedSession): Promise<AgentInstance> {
    if (!managed.agent) {
      ...
      managed.agent = createBackendFromResolvedContext({
        ...
        enable1MContext: await (async () => { const { getEnable1MContext } = await import('@rox-agent/shared/config/storage'); return getEnable1MContext(); })(),
