...
export async function startHeadlessServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.ROX_SERVER_TOKEN
  if (!serverToken) {
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.ROX_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.ROX_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.ROX_RPC_PORT ?? '9100', 10)
  ...
  platform.logger.info(`Rox Agent headless server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
