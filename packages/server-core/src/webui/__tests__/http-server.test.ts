...
afterEach(() => {
  delete process.env.CRAFT_TRUST_FORWARDED_HEADERS
  delete process.env.CRAFT_WEBUI_TRUSTED_PROXIES
...
describe('startWebuiHttpServer', () => {
  ...
  it('infers secure cookies from proxy https headers when no override is set', async () => {
    process.env.CRAFT_TRUST_FORWARDED_HEADERS = '1'
    process.env.CRAFT_WEBUI_TRUSTED_PROXIES = '*'
    const { baseUrl } = await createServer({ wsProtocol: 'wss', wsPort: 9100 })
