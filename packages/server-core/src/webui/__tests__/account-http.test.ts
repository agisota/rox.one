...
describe('account webui auth', () => {
  ...
  it('exposes teams, spaces, and invite aliases with deny-by-default RBAC', async () => {
    ...
    const ownerRoleInvite = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamId}/invites`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', cookie: ownerCookie },
    ...
    const invalidRoleInvite = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamId}/invites`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', cookie: ownerCookie },
  ...
  it('queries stored team audit records with filters while preserving team audit RBAC', async () => {
    ...
    const query = new URL(`http://craft.test/api/account/teams/${teamId}/audit`)
  ...
  it('shares team cloud workspaces with team members and keeps outsiders denied', async () => {
    ...
    const memberLogin = await handler.fetch(new Request('http://craft.test/api/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'member@example.com', password: 'password123' }),
    }))
    ...
    const createWorkspace = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamId}/workspaces`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', cookie: ownerCookie },
    ...
    const memberWorkspaces = await handler.fetch(new Request('http://craft.test/api/account/workspaces', {
      headers: { cookie: memberCookie },
    }))
    ...
    const memberMe = await handler.fetch(new Request('http://craft.test/api/account/me', {
      headers: { cookie: memberCookie },
    }))
    ...
    const memberCreate = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamId}/workspaces`, {
      method: 'POST',
    ...
    const outsiderCookie = outsiderLogin.headers.get('set-cookie') ?? ''
    const outsiderTeamWorkspaces = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamId}/workspaces`, {
      headers: { cookie: outsiderCookie },
    }))
    expect(outsiderTeamWorkspaces.status).toBe(403)

    const outsiderWorkspaces = await handler.fetch(new Request('http://craft.test/api/account/workspaces', {
      headers: { cookie: outsiderCookie },
    }))
  ...
  it('routes managed workspace sync endpoints through an injected service after account access checks', async () => {
    ...
    const anonymous = await handler.fetch(new Request(`http://craft.test/api/account/workspaces/${workspace.id}/sync/status`))
    ...
    const status = await handler.fetch(new Request(`http://craft.test/api/account/workspaces/${workspace.id}/sync/status`, {
      headers: { cookie },
    }))
    expect(status.status).toBe(200)
    expect(await status.json()).toMatchObject({ workspaceId: workspace.id, operationCount: 0 })

    const push = await handler.fetch(new Request(`http://craft.test/api/account/workspaces/${workspace.id}/sync/push`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', cookie },
      body: JSON.stringify({
    ...
    const operations = await handler.fetch(new Request(`http://craft.test/api/account/workspaces/${workspace.id}/sync/operations`, {
      headers: { cookie },
    }))
