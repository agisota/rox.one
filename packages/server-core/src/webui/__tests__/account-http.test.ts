...
describe('account webui auth', () => {
  ...
  it('reports legacy auth as a local session boundary without cloud access', async () => {
    ...
    const auth = await handler.fetch(new Request('http://rox.test/api/auth', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('returns protected cabinet defaults for billing, events, and organizations', async () => {
    ...
    const storage = await handler.fetch(new Request('http://rox.test/api/account/storage', {
      headers: { cookie },
    }))
  ...
  it('exposes teams, spaces, and invite aliases with deny-by-default RBAC', async () => {
    ...
    const createSpace = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/spaces`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', cookie: ownerCookie },
    ...
    const accept = await handler.fetch(new Request(`http://rox.test/api/account/invites/${inviteBody.invite.code}/accept`, {
      method: 'POST',
    ...
    const viewerSpaces = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/spaces`, {
      headers: { cookie: viewerCookie },
    }))
    expect(viewerSpaces.status).toBe(200)

    const viewerCreateSpace = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/spaces`, {
      method: 'POST',
    ...
    const outsiderSpaces = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/spaces`, {
      headers: { cookie: outsiderCookie },
    }))
    expect(outsiderSpaces.status).toBe(403)

    const reused = await handler.fetch(new Request(`http://rox.test/api/account/invites/${inviteBody.invite.code}/accept`, {
      method: 'POST',
      headers: { cookie: outsiderCookie },
    }))
    expect(reused.status).toBe(400)
  ...
  it('exposes team audit events only to owner/admin memberships', async () => {
    ...
    const ownerAudit = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/audit`, {
    ...
    const viewerAudit = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/audit`, {
      headers: { cookie: viewerCookie },
    }))
    ...
    const outsiderAudit = await handler.fetch(new Request(`http://rox.test/api/account/teams/${teamId}/audit`, {
      headers: { cookie: outsiderCookie },
    }))
  ...
  it('creates DV.net top-up intents and credits confirmed webhooks once', async () => {
    ...
    const webhook = await handler.fetch(new Request('http://rox.test/api/webhooks/dvnet', {
      method: 'POST',
    ...
    const duplicate = await handler.fetch(new Request('http://rox.test/api/webhooks/dvnet', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-sign': signature },
      body: rawBody,
    }))
    ...
    const invalid = await handler.fetch(new Request('http://rox.test/api/webhooks/dvnet', {
      method: 'POST',
    ...
    const oversized = await handler.fetch(new Request('http://rox.test/api/webhooks/dvnet', {
      method: 'POST',
