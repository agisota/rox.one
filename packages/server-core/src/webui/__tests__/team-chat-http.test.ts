...
describe('team chat HTTP collaboration routes', () => {
  ...
  it('denies team chat workspace spoofing across team workspace boundaries', async () => {
    ...
    const accepted = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamAlpha.id}/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        cookie,
    ...
    const foreignWorkspace = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamAlpha.id}/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        cookie,
    ...
    const foreignRef = await handler.fetch(new Request(`http://craft.test/api/account/teams/${teamAlpha.id}/messages`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        cookie,
    ...
    const foreignList = await handler.fetch(new Request(
      `http://craft.test/api/account/teams/${teamAlpha.id}/messages?workspaceId=${betaWorkspace.id}`,
      { headers: { cookie } },
    ))
