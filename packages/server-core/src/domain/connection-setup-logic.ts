...
export function parseTestConnectionError(msg: string): string {
  ...
  if (lower.includes('no api key found for')) {
    return 'Provider mismatch during setup. Select a provider preset in ROX ONE Backend API Key mode, or use Anthropic API Key mode for arbitrary compatible endpoints.'
  }
...
export function validateSetupTestInput(params: {
  ...
  if (params.provider === 'pi' && hasCustomEndpoint && !params.piAuthProvider) {
    return {
      valid: false,
      error: 'Custom endpoint in ROX ONE Backend mode requires selecting a provider preset. For arbitrary Anthropic-compatible endpoints, use Anthropic API Key mode.',
    }
  }
...
export function resolveLlmEndpointDependencyRiskMode(
  ...
  const llmSpecific = parseLlmEndpointDependencyRiskMode(
    env.CRAFT_LLM_ENDPOINT_DEPENDENCY_RISK_MODE,
    'CRAFT_LLM_ENDPOINT_DEPENDENCY_RISK_MODE',
  )
  if (llmSpecific) return llmSpecific

  const generic = parseLlmEndpointDependencyRiskMode(
    env.CRAFT_PROVIDER_DEPENDENCY_RISK_MODE,
    'CRAFT_PROVIDER_DEPENDENCY_RISK_MODE',
  )
  if (generic) return generic

  return env.CRAFT_PUBLIC_APP_URL?.trim() ? 'public-untrusted' : 'private-local'
}
...
export function resolveLlmProviderDependencyRiskMode(
  ...
  const llmSpecific = parseLlmEndpointDependencyRiskMode(
    env.CRAFT_LLM_PROVIDER_DEPENDENCY_RISK_MODE,
