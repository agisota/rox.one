...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (readEnv('ROX_DEBUG') === 'true' || process.env.ROX_IS_PACKAGED !== 'true') {
 ...
 * - ROX_APP_ROOT — override appRootPath (default: cwd)
 * - ROX_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
...
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  ...
  const isDebugMode = readEnv('ROX_DEBUG') === 'true' || process.env.ROX_IS_PACKAGED !== 'true'
