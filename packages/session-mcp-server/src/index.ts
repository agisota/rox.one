 ...
 *   --workspace-root: Path to workspace folder (~/.rox-agent/workspaces/{id})
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.rox-agent/workspaces/{id}
      // preferencesPath = ~/.rox-agent/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.ROX_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
// ============================================================
// Rox Agents Docs Upstream Proxy
// ============================================================

const DOCS_MCP_URL = 'https://agents.rox.do/docs/mcp';
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'rox-agent-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`Rox Agents Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Rox Agents Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Rox Agents Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (ROX_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (ROX_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.ROX_LLM_CALLBACK_PORT,
  };
